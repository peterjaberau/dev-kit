...
const GraphGrid = React.memo((props: any) => (
  <Box className="graph-grid-placeholder" {...props} />
));
...
const TokenGraphAdjacency = React.memo((props: any) => (
  <Box className="adjacency-placeholder" />
));
...
export const TokenGraph: React.FC<TokenGraphProps> = ({
  ...
  const debouncedAfterWheel = useMemo(
    () => debounce(_debouncedAfterWheel, 150),
  ...
  const handleZoomGesture = useCallback(
    (e: React.GestureEvent | React.WheelEvent) => {
      let newZoom = 0;
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (e.button === 0) { // Only primary mouse button (left)
  ...
  const renderAdjacencies = useMemo(() => {
    return adjacencyTuples.map((tuple) => {
      ...
      const isOnAncestorPath =
        selectionAncestorNodes.includes(fromNode.id) &&
        selectionAncestorNodes.includes(toNode.id);
      const isOnDescendentPath =
        selectionDescendentNodes.includes(fromNode.id) &&
        selectionDescendentNodes.includes(toNode.id);
      const isInHoverUpstream =
        renderedHoverUpstream.includes(fromNode.id) &&
        renderedHoverUpstream.includes(toNode.id);

      const isFocused =
        focusedOrSelected.includes(fromId) && focusedOrSelected.includes(toId);
  ...
  const renderNodes = useMemo(() => {
    return nodeIds.map((id) => {
      ...
      // Transform style for node positioning
