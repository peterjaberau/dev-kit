...
export class AppController {
  ...
  async initialize({
    ...
    const selectedComponentIds =
      encodedSelectedComponents === "ALL" ? [...this.graphController.listOfComponents] : selectedComponents
  ...
  handleNewGraphState() {
    ...
    const newTopologyKey = this.graphController.displayGraphModel._state.topologyKey + isFullscreenMode
    ...
    const descendents = this.graphController.getDescendentNodes(...selectedTokens, ...selectedComponents)
    const descendentIntersect = this.graphController.getDescendentIntersectNodes(
      ...selectedTokens,
      ...selectedComponents,
    )
    ...
    // the viewport...
    const allSelections = [...selectedTokens, ...selectedComponents, ...descendents, ...ancestors]
    ...
    allSelections.forEach((id) => {
      ...
      if (node) {
        ...
        foundMaxX = Math.max(foundMaxX || node.x, node.x + 50 + GRAPH_NODE_WIDTH)
        foundMaxY = Math.max(foundMaxY || node.y, node.y + 50 + GRAPH_NODE_HEIGHT)
      }
    })
  ...
  handleHoverOver(nodeId: string) {
    ...
    const currentSelectionAncestors = this.appModel._state.selectionAncestorNodes
    const currentSelectionDescendents = this.appModel._state.selectionDescendentNodes
    const displayedGraphNodes = currentSelectionAncestors.concat(currentSelectionDescendents)
    const result = ancestors.filter((graphNodeId) => displayedGraphNodes.includes(graphNodeId))
  ...
  toggleGraphNodeSelection(graphNodeId: string, modifierKey: boolean = false) {
    const node = this.graphController.completeGraphModel._state.nodes[graphNodeId]
    ...
    const deselectingThisId = selectedComponents.indexOf(graphNodeId) >= 0 || selectedTokens.indexOf(graphNodeId) >= 0
    ...
    const descendents = this.graphController.getDescendentNodes(...allSelections)
    const descendentIntersect = this.graphController.getDescendentIntersectNodes(...allSelections)
