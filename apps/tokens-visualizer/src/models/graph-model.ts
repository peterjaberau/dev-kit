...
export class GraphModel {
  ...
  filter(fn: (node: GraphNode) => boolean): GraphModel {
    ...
    nodeIds.forEach((id) => {
      if (fn(this._state.nodes[id] as any)) {
        result.createNode(JSON.parse(JSON.stringify(this._state.nodes[id])));
      }
    });
    adjacencyKeys.forEach((id) => {
      if (result._state.nodes[id]) {
        const adjacencyTargets: any = this._state.adjacencyList[id];
        adjacencyTargets.forEach((targetId: any) => {
  ...
  deleteNode(id: GraphNodeId) {
    ...
    for (const fromNodeId in this._state.adjacencyList) {
      const toIndex = (this._state.adjacencyList[fromNodeId] as any).indexOf(id);
      if (toIndex >= 0) {
        (this._state.adjacencyList[fromNodeId] as any).splice(toIndex, 1);
      }
    }
