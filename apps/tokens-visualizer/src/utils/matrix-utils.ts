...
export class MatrixUtils {
  ...
  static scale(out: number[] | any, a: number[] | any, v: number[] | any) {
    const a0 = a[0];
    const a1 = a[1];
    const a2 = a[2];
    const a3 = a[3];
  ...
  static rotate(out: number[], a: number[], rad: number) {
    ...
    const a4: any = a[4];
    const a5: any = a[5];
    const s: any = Math.sin(rad);
    const c: any = Math.cos(rad);
    out[0] = a0 * c + a2 * s;
    out[1] = a1 * c + a3 * s;
    out[2] = a0 * -s + a2 * c;
    out[3] = a1 * -s + a3 * c;
  ...
  static translate(out: number[] | any, a: number[] | any, v: number[] | any) {
    const a0 = a[0];
    const a1 = a[1];
    const a2 = a[2];
    const a3 = a[3];
  ...
  static transformPoint(x: number| any, y: number| any, m: number[]| any) {
    return {
      x: x * m[0] + y * m[2] + m[4],
      y: x * m[1] + y * m[3] + m[5],
    };
  ...
  static decompose(m: number[]) {
    ...
    const tx: any = m[4];
    const ty: any = m[5];
    const skewX: any = -Math.atan2(-c, d);
    const skewY: any = Math.atan2(b, a);
    const delta: any = Math.abs(skewX + skewY);
    const isNotZeroRotation: any =
