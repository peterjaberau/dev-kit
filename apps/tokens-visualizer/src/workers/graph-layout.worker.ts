...
function graphLayout(graphState: GraphState) {
  ...
  const orphanNodes = graphModel.orphanNodes().sort((a: any, b: any) => {
    const nodeA: any = graphState.nodes[a];
    const nodeB: any = graphState.nodes[b];
  ...
  const nodesToLayout = orphanNodes.map((orphanId: any) => {
  ...
  nodesToLayout.sort((a: any, b: any) => {
    const d1 = a[1];
    const d2 = b[1];
    if (d1 === d2) {
      return 0;
  ...
  // return;
  const columnNodeAssignments: string[][] | any = [];
  ...
  while (nodesToLayout.length > 0) {
    ...
    const node: any = graphModel._state.nodes[id];
  ...
  for (let index = 0; index <= columnInsertionPoints.length - 1; index++) {
    // console.info(index);
    const columnHeight: any = columnInsertionPoints[index];
    columnOffsets[index] = (maxHeight - columnHeight) / 2;
  }

  let priorColumnAncestorYValues = [0];

  columnNodeAssignments.forEach((assignments: any, colIndex: any) => {
