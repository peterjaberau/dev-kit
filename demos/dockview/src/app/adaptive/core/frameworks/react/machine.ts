...
export function useMachine(
  ...
  const previousEventRef: any = useRef(null)
  ...
  const getState = () => ({
    ...
    hasTag(tag: any) {
      return !!machine.states[state.ref.current as any]?.tags?.includes(tag)
    },
  })

  const refs: any = useRefs(machine.refs?.({ prop, context: ctx }) ?? {})
  ...
  const action = (keys: any | undefined) => {
  ...
  const effect = (keys: any | undefined) => {
    const strs = isFunction(keys) ? keys(getParams()) : keys
    if (!strs) return
    const fns = strs.map((s: any) => {
  ...
  const choose: any = (transitions: any) => {
...
function useLiveRef(value: any) {
  const ref = useRef(value)
  ref.current = value
  return ref
}

function useProp(value: any) {
  const ref = useLiveRef(value)
  return function get(key: any) {
