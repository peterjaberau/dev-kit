...
export function expandBranches(params: any, values: string[]) {
  const { context, prop, refs } = params

  if (!prop("loadChildren")) {
    context.set("expandedValue", (prev: any) => uniq(add(prev, ...values)))
    return
  }
  ...
  if (loadedValues.length > 0) {
    context.set("expandedValue", (prev: any) => uniq(add(prev, ...loadedValues)))
  }
  ...
  if (nodeWithChildren.length > 0) {
    context.set("expandedValue", (prev: any) => uniq(add(prev, ...nodeWithChildren)))
  }

  if (nodeWithoutChildren.length === 0) return

  context.set("loadingStatus", (prev: any) => ({
    ...prev,
