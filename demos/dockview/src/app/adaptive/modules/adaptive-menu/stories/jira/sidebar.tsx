...
export function Sidebar() {
      ...
      onDrop({ source, location }) {
        ...
        if (isTopLevelItemData(dragging) && isTopLevelItemData(dropTargetData)) {
          setData((current) => {
            ...
            const withoutDraggingItem = current.topLevelItems.filter((item) => item !== dragging.value)
        ...
        if (isProjectData(dragging) && isProjectData(dropTargetData)) {
          ...
          setData((current) => {
            ...
            const indexOfTarget = withoutDraggingProject.findIndex((project) => project.id === dropTargetData.id)
  ...
  useEffect(() => {
    ...
    if (action.type === "reorder-project") {
      ...
      const project = stableData.current.projects[action.groupName].find((project) => project.id === action.draggingId)
      invariant(project)
          ...
          <LastActionContext.Provider value={data.lastAction}>
            <RegistryContext.Provider value={registry}>
              <AdaptiveMenu.MenuList>
                <AdaptiveMenu.GroupDropIndicator ref={ref} isActive={state === "is-over"}>
                ...
                </AdaptiveMenu.GroupDropIndicator>
              </AdaptiveMenu.MenuList>
