...
import { JSONColorFormat, JSONJSONFormat } from "@jsonhero/json-infer-types/lib/formats"
    ...
    case "uri":
      if (
        ...
        if (url.protocol === "ipfs:") {
          ...
          return <PreviewImageUri src={info.value} contentType={info.format.contentType} />
        }
      } else if (
        ...
        return <PreviewVideoUri src={info.value} contentType={info.format.contentType} />
      } else if (
        info.format.contentType === "audio/mpeg" ||
        info.format.contentType === "audio/ogg" ||
        info.format.contentType === "audio/wav"
      ) {
        return <PreviewAudioUri src={info.value} contentType={info.format.contentType} />
      } else {
...
function PreviewJson({ value, format }: { value: string; format: JSONJSONFormat }) {
...
function PreviewColor({ value, format }: { value: string; format: JSONColorFormat }) {
