...
export function useColumnView({
  ...
  const nodeTable: any = useMemo<NodeTable>(() => generateNodeTable(rootNode), [rootNode])

  const enhancedReducer: Reducer<ColumnViewState, ColumnViewAction> = useCallback(
  ...
  const { selectedNodeId, highlightedNodeId, selectedNodeSource, history, historyCurrentIndex } = state
