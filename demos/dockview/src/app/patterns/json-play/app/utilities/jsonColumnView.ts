...
//ColumnViewNode
export function generateColumnViewNode(json: unknown) {
...
export function calculateStablePath(
  ...
  for (let index = 0; index < selectionPath.components.length; index++) {
    const selectionComponent: any = selectionPath.components[index];
    const previousComponent: any = previousPath.components[index]
