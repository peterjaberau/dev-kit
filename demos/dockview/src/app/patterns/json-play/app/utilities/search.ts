...
export function getComponentSlices(
  ...
  const calculateLongestMatch = (componentSlices: ComponentSlice[]): number => {
    return componentSlices.reduce(
      (longestMatch, slice) => (slice.slice.isMatch ? Math.max(longestMatch, slice.slice.slice.length) : longestMatch),
      0,
    )
  }

  const addEllipsisToSlices = (slices: PathSlice[], mostImportantComponentIndex: number): PathSlice[] => {
    ...
    const replaceComponentIndexWithEllipsis = (ellipsisComponentIndex: number): PathSlice[] => {
      const ellipsisSliceIndices = slices
        .map((slice, index) => [slice, index] as [PathSlice, number])
        .filter(([slice, index]) => slice.type === "component" && slice.componentIndex === ellipsisComponentIndex)
        .map(([, index]) => index)
    ...
    //@ts-ignore
    const componentSlices: any = Array.keepMap(slices, (slice: any) => (slice.type === "component" ? slice : null))

    const componentIndexes = uniq(componentSlices.map((slice: any) => slice.componentIndex))

    const ellipsisIndex = slices.findIndex((slice) => slice.type === "ellipsis")

    if (ellipsisIndex === -1) {
      let ellipsisComponentIndex: any = 0
      ...
      //@ts-ignore
      const nearestBefore = Array.keepMap(slices.slice(0, ellipsisIndex).reverse(), (slice: any) =>
        slice.type === "component" ? slice : null,
      )[0]

      //@ts-ignore
      const nearestAfter = Array.keepMap(slices.slice(ellipsisIndex + 1), (slice: any) =>
        slice.type === "component" ? slice : null,
      )[0]
  ...
  while (weight > maxWeight) {
    //@ts-ignore
    const componentSlices = Array.keepMap(slices, (slice: any) => (slice.type === "component" ? slice : null))

    const groupByComponentIndex: any = groupBy(componentSlices, (slice) => slice.componentIndex)

    const sortedByLongestMatch: any = Dict.entries(groupByComponentIndex).sort(
