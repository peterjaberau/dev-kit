...
export function SortableTree<T>({
  ...
  function handleDragEnd({ active, over }: DragEndEvent) {
    ...
    if (projected && over) {
      ...
      const clonedItems: FlattenedItem<T>[] | any = JSON.parse(JSON.stringify(flattenTree(value)));
      const overIndex = clonedItems.findIndex(({ id }: any) => id === over.id)
      const activeIndex = clonedItems.findIndex(({ id }: any) => id === active.id)
      ...
      const depthItems = sortedItems.filter(({ parentId }: any) => parentId === clonedItems[activeIndex].parentId)

      const currentDepthIndex = depthItems.findIndex(({ id }: any) => id === clonedItems[activeIndex].id)
  ...
  function resetState() {
    ...
    setActiveNode(null);
    setOffsetLeft(0);
    document && document.body.style.setProperty("cursor", defaultBodyCursor.current);
    document &&  document.body.style.setProperty("user-select", defaultBodyUserSelect.current)
  }
      ...
      <SortableContext items={sortedIds} strategy={verticalListSortingStrategy}>
        {virtual ? (
          <VirtualList
            ...
            className={virtual?.hideScrollbar ? "virtualListWrapper" : undefined}
            stickyIndices={activeNode ? [flattenedItems.findIndex(({ id }) => id === activeNode.id)] : undefined}
            onScroll={(_, e) => {
            ...
            renderItem={({ index, style }) => {
              ...
              return (
                <div key={index} style={{ ...style, position: "absolute" }}>
                  {renderSortableItem(node, activeNode?.id === node.id ? { canMove: projected?.canMove } : undefined)}
                </div>
              )
            }}
          />
