...
export interface Options {
  itemCount: number;
  itemSizeGetter: ItemSizeGetter | any;
...
export default class SizeAndPositionManager {
   ...
   */
  getSizeAndPositionForIndex(index: number): any {
    if (index < 0 || index >= this.itemCount) {
      throw Error(`Requested index ${index} is outside of range 0..${this.itemCount}`)
    }

    if (index > this.lastMeasuredIndex) {
      const lastMeasuredSizeAndPosition: any = this.getSizeAndPositionOfLastMeasuredItem()
  ...
  getUpdatedOffsetForIndex({
    ...
    const datum: any = this.getSizeAndPositionForIndex(targetIndex)
  ...
  getVisibleRange({
    ...
    const datum: any = this.getSizeAndPositionForIndex(start)
