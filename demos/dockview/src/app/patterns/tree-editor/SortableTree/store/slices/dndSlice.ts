...
> = (set, get) => ({
  ...
  handleDragOver: ({ over }) => {
    set({ overId: over?.id ?? null } as any);
  },
  handleDragEnd: ({ active, over }: any) => {
    const { resetState, dispatchTreeData, sortableRule } = get();

    const dataFlatten = dataFlattenSelector(get());
    const activeNode: any = dataFlatten.find((i) => i.id === active.id);
    const targetNode: any = dataFlatten.find((i) => i.id === over.id);
    const projected: any = projectedSelector(get());
