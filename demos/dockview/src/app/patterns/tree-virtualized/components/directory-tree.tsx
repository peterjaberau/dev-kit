...
const DragPreview = ({ count, firstId, nodes }: { count: number, firstId: string, nodes: DirectoryNode[] }|any) => {
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    if (over.id === "ROOT") {
      if (flattenedNodes.length > 0) {
        const lastNode: any = flattenedNodes[flattenedNodes.length - 1];
