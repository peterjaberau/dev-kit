...
import { getCheckedState, getCheckedValueMap } from "../shared"

export function connect(service: any, normalize: any) {
  const { context, scope, computed, prop, send } = service
  const collection = prop("collection")

  const expandedValue: any = Array.from(context.get("expandedValue"))
  const selectedValue: any = Array.from(context.get("selectedValue"))
  const checkedValue: any = Array.from(context.get("checkedValue"))
  ...
  function getNodeState(props: any) {
  ...
  return {
    ...
    selectedValue,
    checkedValue,
    toggleChecked(value: any, isBranch: any) {
    ...
    startRenaming(value: any) {
      send({ type: "NODE.RENAME", value })
    },
    submitRenaming(value: any, label: any) {
    ...
    getItemIndicatorProps(props: any) {
      const itemState = getNodeState(props)
      return normalize.element({
    ...
    getBranchIndicatorProps(props: any) {
      const nodeState = getNodeState(props)
      return normalize.element({
    ...
    getBranchTriggerProps(props: any) {
      const nodeState = getNodeState(props)
      return normalize.element({
    ...
    getBranchControlProps(props: any) {
      const nodeState = getNodeState(props)
      return normalize.element({
    ...
    getBranchContentProps(props: any) {
      const nodeState = getNodeState(props)
      return normalize.element({
    ...
    getBranchIndentGuideProps(props: any) {
      const nodeState = getNodeState(props)
      return normalize.element({
    ...
    getNodeCheckboxProps(props: any) {
      const nodeState = getNodeState(props)
    ...
    getNodeRenameInputProps(props: any) {
      const nodeState = getNodeState(props)
