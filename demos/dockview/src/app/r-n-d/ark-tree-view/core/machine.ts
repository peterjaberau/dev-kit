...
import { expandBranches, skipFn } from "./utils"
import { toggleBranchChecked,  } from "../shared/checked-state"
...
export const machine = createMachine({
  ...
  computed: {
    isMultipleSelection: ({ prop }: any) => prop("selectionMode") === "multiple",
    isTypingAhead: ({ refs }: any) => refs.get("typeaheadState").keysSoFar.length > 0,
    visibleNodes: ({ prop, context }: any) => {
      const nodes: { node: any; indexPath: number[] | any[] }[] = []
  ...
  implementations: {
    guards: {
      isBranchFocused: ({ context, event }: any) => context.get("focusedValue") === event.id,
      isBranchExpanded: ({ context, event }: any) => context.get("expandedValue").includes(event.id),
      isShiftKey: ({ event }) => event.shiftKey,
      isCtrlKey: ({ event }) => event.ctrlKey,
      hasSelectedItems: ({ context }: any) => context.get("selectedValue").length > 0,
      isMultipleSelection: ({ prop }: any) => prop("selectionMode") === "multiple",
      moveFocus: ({ event }) => !!event.moveFocus,
      expandOnClick: ({ prop }: any) => !!prop("expandOnClick"),
      isRenameLabelValid: ({ event }) => event.label.trim() !== "",
    },
    actions: {
      selectNode({ context, event }: any) {
        const value = (event.id || event.value) as string | string[] | undefined
        context.set("selectedValue", (prev: any) => {
      ...
      deselectNode({ context, event }: any) {
        const value = toArray(event.id || event.value)
        context.set("selectedValue", (prev: any) => remove(prev, ...value))
      },
      setFocusedNode({ context, event }: any) {
        context.set("focusedValue", event.id)
      },
      clearFocusedNode({ context }: any) {
        context.set("focusedValue", null)
      },
      clearSelectedItem({ context }: any) {
        context.set("selectedValue", [])
      },
      toggleBranchNode({ context, event, action }: any) {
        const isExpanded = context.get("expandedValue").includes(event.id)
        action(isExpanded ? ["collapseBranch"] : ["expandBranch"])
      },
      expandBranch(params: any) {
        const { event } = params
        expandBranches(params, [event.id])
      },
      expandBranches(params: any) {
        const { context, event } = params
        const valuesToExpand = toArray(event.value)
        expandBranches(params, diff(valuesToExpand, context.get("expandedValue")))
      },
      collapseBranch({ context, event }: any) {
        context.set("expandedValue", (prev: any) => remove(prev, event.id))
      },
      collapseBranches(params: any) {
        const { context, event } = params
        const value = toArray(event.value)
        context.set("expandedValue", (prev: any) => remove(prev, ...value))
      },
      setExpanded({ context, event }: any) {
        if (!isArray(event.value)) return
        context.set("expandedValue", event.value)
      },
      clearExpanded({ context }: any) {
        context.set("expandedValue", [])
      },
      setSelected({ context, event }: any) {
        if (!isArray(event.value)) return
        context.set("selectedValue", event.value)
      },
      clearSelected({ context }: any) {
        context.set("selectedValue", [])
      },
      focusTreeFirstNode(params: any) {
      ...
      focusBranchFirstNode(params: any) {
      ...
      selectAllNodes({ context, prop }: any) {
        context.set("selectedValue", prop("collection").getValues())
      },
      focusMatchedNode(params: any) {
      ...
      expandSiblingBranches(params: any) {
        ...
        const values = nodes.map((node: any) => collection.getNodeValue(node))

        const valuesToExpand: any = diff(values, context.get("expandedValue"))
        expandBranches(params, valuesToExpand)
      },
      extendSelectionToNode(params: any) {
      ...
      extendSelectionToNextNode(params: any) {
        const { context, event, prop } = params
        const collection = prop("collection")
        const nextNode = collection.getNextNode(event.id, { skip: skipFn(params) })
        if (!nextNode) return
      ...
      extendSelectionToPrevNode(params: any) {
        const { context, event, prop } = params
        const collection = prop("collection")
        const prevNode = collection.getPreviousNode(event.id, { skip: skipFn(params) })
        if (!prevNode) return
      ...
      extendSelectionToFirstNode(params: any) {
        const { context, prop } = params
        const collection = prop("collection")
      ...
      extendSelectionToLastNode(params: any) {
        const { context, prop } = params
        const collection = prop("collection")
        const currentSelection = first(context.get("selectedValue"))
        const values: string[] = []
      ...
      clearPendingAborts({ refs }: any) {
        const aborts = refs.get("pendingAborts")
        aborts.forEach((abort: any) => abort.abort())
        aborts.clear()
      },
      toggleChecked({ context, event, prop }: any) {
        const collection = prop("collection")
        context.set("checkedValue", (prev: any) =>
          event.isBranch ? toggleBranchChecked(collection, event.value, prev) : addOrRemove(prev, event.value),
        )
      },
      setChecked({ context, event }: any) {
        context.set("checkedValue", event.value)
      },
      clearChecked({ context }: any) {
        context.set("checkedValue", [])
      },
      setRenamingValue({ context, event, prop }: any) {
      ...
      submitRenaming({ context, event, prop, scope }: any) {
      ...
      cancelRenaming({ context, scope }: any) {
        const renamingValue = context.get("renamingValue")
        context.set("renamingValue", null)
        if (renamingValue) {
          dom.focusNode(scope, renamingValue)
      ...
      syncRenameInput({ context, scope, prop }: any) {
        const renamingValue = context.get("renamingValue")
        if (!renamingValue) return
      ...
      focusRenameInput({ context, scope }: any) {
        const renamingValue = context.get("renamingValue")
        if (!renamingValue) return
...
function scrollToNode(params: any, value: string): boolean {
