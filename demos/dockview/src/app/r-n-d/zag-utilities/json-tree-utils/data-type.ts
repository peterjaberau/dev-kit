...
const txt = (value: string | number | boolean | null | undefined) => ({
  type: "text",
  value,
})

const jsx = (tagName: "span" | "div" | "a", properties: any["properties"] = {}, children: any[] = []) => ({
  type: "element",
  tagName,
  properties,
  children,
...
const formatValueMini = (child: any): string => {
...
function dataType<T = unknown>(opts: any) {
  return opts
}
...
export const SetType = dataType<Set<unknown>>({
  ...
  previewElement(node: any, opts: any) {
...
export const URLSearchParamsType = dataType<URLSearchParams>({
  ...
  node({ value, keyPath, createNode }: any) {
    const entriesChildren = Array.from(value.entries()).map(([key, value]: any, index): any => {
...
const getFunctionSignature = (func: Function): string => {
  ...
  const signatureMatch: any = funcString.match(FUNCTION_SIGNATURE_REGEX)
...
export const FunctionType = dataType<Function>({
  ...
  node({ value, keyPath, createNode, options }: any) {
...
function errorStackToElement(stack: string): any {
...
export const ArrayType = dataType<Array<unknown>>({
  ...
  previewText(node: any, opts: any) {
    ...
    const enumerableChildren = children.filter(
      (child: any) => !child.isNonEnumerable && keyPathToKey(child.keyPath) !== "length",
    )
  ...
  node({ value, keyPath, createNode, options }: any) {
    ...
    if (shouldGroup) {
      ...
      for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        const chunk: any = chunks[chunkIndex]
        ...
        const groupChildren = chunk.map((index: any) => createNode([index.toString()], value[index]))

        // Create the group node
        const groupNode: any = {
          keyPath: [...keyPath, groupKey],
          value: chunk.map((index: any) => value[index]),
          type: "array",
...
export const TypedArrayType = dataType<any>({
  type: (value: any) => Reflect.get(typedArrayConstructors, value.constructor.name),
  description(node: any) {
...
export const ObjectType = dataType<object>({
  ...
  previewText(node: any, opts: any) {
    const maxItems = opts.maxPreviewItems
    const children = node.children || []
    const previews = children.slice(0, maxItems).map((child: any) => {
  ...
  node({ value, keyPath, createNode }: any) {
    const children = ELEMENT_KEYS.reduce((acc, key) => {
      ...
      if (key === "attributes") {
        ...
        childValue = attrs.length ? Object.fromEntries(attrs.map((attr: any) => [attr.name, attr.value])) : undefined
      }
...
export const StringType = dataType<string>({
  ...
  previewText(node: any, opts: any) {
    const serialised = node.value.replace(STRING_ESCAPE_REGEXP, (_: string): any => map[_])
    const preview =
...
export const jsonNodeToElement = (node: any, opts?: any): any => {
...
export const getNodeTypeDescription = (node: any, opts?: any): string => {
  const options = getPreviewOptions(opts)
  const dataType = dataTypes.find((dataType) => dataType.check(node.value))
  if (dataType) {
