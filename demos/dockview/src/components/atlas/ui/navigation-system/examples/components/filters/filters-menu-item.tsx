...
import { LuListFilter as FilterIcon, LuPlus as AddIcon  } from "react-icons/lu";
...
} from '#atlas-ui/navigation-system/app/entry-points/side-nav-items/expandable-menu-item';
import { LinkMenuItem } from '#atlas-ui/navigation-system/app/entry-points/side-nav-items/link-menu-item';
import {
	...
	return (
        ...
        {/* @ts-ignore */}
				<ExpandableMenuItemTrigger
					...
					hasDragIndicator
					elemBefore={() => <Icon size={'xs'}><FilterIcon /></Icon>}
					actionsOnHover={
            <IconButton
              size="xs"
              variant={'subtle'}
...
function FilterLeaf({ filter }: { filter: any }) {
...
function FilterParent({ filter }: { filter: any }) {
...
function FilterList({ filters }: { filters: any[] }) {
