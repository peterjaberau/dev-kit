...
import { isObject, isArray, isBoolean, isNumber, isString } from '../utils'

export const nodeTreeLeafMachine = setup({
  types: {
    context: {} as any,
    events: {} as any,
  } as any,
  ...
  guards: {
    ...
    // }),
    // isBranchArrayNode: (({ context }: any) => {
    //   const data = context?.config?.data
    //   return isObject(data)
    // }),
    // isBranchObjectNode: (({ context }: any) => {
    //   const data = context?.config?.data
    //   return isArray(data)
    // }),
