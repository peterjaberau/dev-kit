...
import { IconButton, HStack, Wrap, WrapItem } from "@chakra-ui/react"
import {
  customCopy,
  customDelete,
  customEdit,
      ...
      <>
        ...
        {isEditing &&
          (typeof CancelComponent === "function" ? (
            <CancelComponent css={{ display: "inline-block" }} onClick={cancel} />
          ) : (
        ...
        {!isEditing &&
          ...
          type === "string" &&
          urlRegExp.test(node) &&
          customMatchesURL(customReturn as CustomizeOptions | undefined) && (
        ...
        {!isEditing &&
          ...
          (typeof EditComponent === "function" ? (
            <EditComponent  onClick={edit} />
          ) : (
            <IconButton size="2xs" variant="ghost" onClick={edit}>
              <EditIcon />
            </IconButton>
          ))}
        {!isEditing &&
        ...
        <HStack w="full">
          <HStack flex={1}>
            {editing ? (
              ...
              <LongString str={node} ref={valueRef} className={''} ctrlClick={ctrlClick} />
            ) : (
              <span className={''} onClick={ctrlClick}>
                "{displayValue}"
              </span>
            )}
          </HStack>
