...
import { IconButton, useSlotRecipe, HStack, Wrap, WrapItem } from '@chakra-ui/react'
import {
	customCopy,
	customDelete,
	customEdit,
...
export default function JsonNode({ node, depth, deleteHandle: _deleteHandle, indexOrName, parent, editHandle, parentPath }: Props) {
  ...
  const { copy: cssCopy, edit: cssEdit, deleting: cssDeleting, link: cssLink, string: cssString, number: cssNumber, boolean: cssBoolean, null: cssNull } = recipe()
	...
	if (Array.isArray(node) || isObject(node)) {
		...
		const Icons = (
            ...
            <IconButton
              ...
              css={{
              ...cssEdit,
              display: 'inline-block' }} onClick={deleting ? deleteHandle : done}>
              <DoneIcon />
            </IconButton>
				...
				{isEditing &&
					(typeof CancelComponent === 'function' ? (
						<CancelComponent className='json-view--edit' css={{ display: 'inline-block' }} onClick={cancel} />
					) : (
            <IconButton size='2xs' variant='ghost' css={{ ...cssEdit, display: 'inline-block' }} onClick={cancel}>
              <CancelIcon />
            </IconButton>
					))}
            ...
            <IconButton css={{...cssEdit}} size='2xs' variant='ghost' onClick={edit}>
              <EditIcon />
            </IconButton>
					))}
				{!isEditing && editableDelete(editable) && customDelete(customReturn as CustomizeOptions | undefined) && _deleteHandle && (

          <IconButton css={{ ...cssEdit }} size='2xs' variant='ghost' onClick={() => setDeleting(true)}>
            <DeleteIcon />
          </IconButton>
