...
import type { Instruction as UnionInstruction } from "../components/drag-drop/drop-target-item"
import { ROOT_PROJECT_GROUP_ID } from "./defaults"
import { isGroup } from "./types/group"
import { createGroupId } from "./types/id"
import { reorderInArray, moveItemBetweenArrays } from "./utils/array-operations"
...
export function findContainingGroup(itemId: any, searchGroup: any): any {
  ...
  const index = searchGroup.items.findIndex((item: any) => (isGroup(item) ? item.id === itemId : item === itemId))
...
export function getGroupById(groupId: any, rootGroup: any): any | null {
 ...
 */
function findGroupByIdRecursive(groupId: any, searchGroup: any): any | null {
...
export function resolveTargetLocation(
  ...
  switch (targetType) {
    case "sidebar-project-drop-target": {
      ...
      const targetGroupId =
        typeof targetData.groupId === "string" ? createGroupId(targetData.groupId) : ROOT_PROJECT_GROUP_ID
 ...
 * ```
 */
export function calculateMove(itemId: any, source: any, target: any, rootGroup: any): any {
