import { createStyles, cx, getStudioStylish } from '#components/pragmatic-drag-drop/tree-editor/theme';
...
import { genUniqueId } from '#components/pragmatic-drag-drop/tree-editor/utils';
...
const TreeNode = memo<TreeNodeProps>(({ node, virtualStyle }) => {
  const [activeId, indentationWidth, dispatchTreeData, hideRemove] = useStore(
    (s: any) => [s.activeId, s.indentationWidth, s.dispatchTreeData, s.hideRemove],
    shallow,
  );
...
const useStyle = createStyles((props: any) => {
...
const TreeList = memo(() => {
  const [dispatchTreeData, hideAdd, virtual] = useStore(
    (s: any) => [s.dispatchTreeData, s.hideAdd, s.virtual],
    shallow,
  );
