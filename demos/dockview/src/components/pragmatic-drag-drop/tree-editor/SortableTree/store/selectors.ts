...
export const dataFlattenSelector = (s: InternalSortableTreeStore | any): FlattenNode[] =>
  getFlattenedData(s.treeData, s.activeId);

export const sortedIdsSelector = (s: InternalSortableTreeStore | any) => {
  return dataFlattenSelector(s).map(({ id }) => id);
};

export const projectedSelector = (s: InternalSortableTreeStore | any) => {
