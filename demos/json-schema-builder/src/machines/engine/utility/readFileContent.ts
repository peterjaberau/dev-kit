 ...
 */
export function readFileContentToDataLink(files: FileList | File[] | null, dataLink: ManagedData): void {
  readFileContentFromFileList(files, true)
    ...
    .catch((error) => console.log("error in readFileContentToDataLink", error))
}

export async function readFileContentToString(files: FileList | File[] | null, result: any) {
  try {
    ...
    console.log("error in readFileContentToString", error)
    // errorService.onError(error);
  }
}

export function readFileContentForFunction(files: FileList | File[] | null, fct: (content: any) => void) {
  readFileContentFromFileList(files, true)
    .then((contents) => {
      if (contents !== undefined) {
        fct(contents as any)
    ...
    .catch((error) => {
      console.log("error in readFileContentForFunction", error)
      // errorService.onError(error);
    })
}
