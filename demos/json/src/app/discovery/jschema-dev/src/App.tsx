...
import { useNodesState, useEdgesState, type Node, type ReactFlowInstance } from "reactflow"
...
const EXAMPLE_SCHEMAS: { path: string; url: string }[] = [
  ...
  {
    path: "v1/notification/notification-settings.schema.json",
    url: "/example-schemas/v1/notification/notification-settings.schema.json",
  },
...
function App() {
  ...
  const highlightedEdges = useMemo(() => {
    ...
    return edges.map((edge) => {
      ...
      if (selectedNodeId) {
        ...
        isConnected =
          sourceNode?.data.entityType === selectedEntityType || targetNode?.data.entityType === selectedEntityType
      }
  ...
  const updateGraph = useCallback(
    async (schemaStr: string) => {
      try {
        const parsed = JSON.parse(schemaStr)
        const { nodes: rawNodes, edges: rawEdges } = schemaToGraph(parsed)
        const { nodes: layoutedNodes, edges: layoutedEdges }: any = await layoutGraph(rawNodes, rawEdges)
  ...
  const handleMultiSchemaChange = useCallback(
    (path: string, content: string) => {
      try {
        ...
        setSchemasMap((prevMap) => {
          ...
          layoutGraph(rawNodes, rawEdges).then(({ nodes: layoutedNodes, edges: layoutedEdges }: any) => {
            setNodes(layoutedNodes)
            setEdges(layoutedEdges)
          })
  ...
  const debouncedMultiSchemaChange = useMemo(() => debounce(handleMultiSchemaChange, 500), [handleMultiSchemaChange])
    ...
    const loadExampleSchemas = async () => {
      ...
      try {
        ...
        if (newSchemasMap.size > 0) {
          ...
          const summary =
            `// Loaded ${newSchemasMap.size} example schemas\n// Found ${allRefs.length} $ref relationships\n\n` +
            `Schemas:\n${Array.from(newSchemasMap.keys())
              .map((name) => `- ${name}`)
              .join("\n")}`
          setSchema(summary)
    ...
    if (urlParam) {
      ...
      const loadFromUrl = async () => {
        ...
        try {
          ...
          const summary =
            `// Loaded ${schemasMap.size} schemas\n// Found ${allRelationships.length} $ref relationships\n\n` +
            `Schemas:\n${Array.from(schemasMap.keys())
              .map((name) => `- ${name}`)
              .join("\n")}`
          setSchema(summary)
          setViewMode("multi")
  ...
  const handleEditorClick = useCallback((property: string) => {
    setNodes((currentNodes) => {
      ...
      console.log(
        "Available nodes:",
        currentNodes.map((n) => ({ id: n.id, label: n.data.label, schemaName: n.data.schemaName })),
      )
      ...
      if (reactFlowInstance.current && currentNodes.length > 0) {
        ...
        if (matchingNode) {
          ...
          reactFlowInstance.current.setCenter(matchingNode.position.x + 100, matchingNode.position.y + 40, {
            zoom: 1,
            duration: 800,
          })
  ...
  const handleLoadAllSchemas = async () => {
    ...
    try {
      ...
      const nodesWithHandlers = layoutedNodes.map((node: any) => ({
        ...node,
        data: {
          ...node.data,
  ...
  const handleNodeClick = useCallback(
    (_event: React.MouseEvent, node: Node) => {
      ...
      if (viewMode === "multi" && node.data.schemaName) {
        ...
        const matchingPath = Array.from(schemasMap.keys()).find(
          (path) => path === schemaName || path.endsWith("/" + schemaName) || path.endsWith(schemaName),
        )
        if (matchingPath) {
  ...
  const handleNavigateBack = useCallback(
    async (index: number) => {
      ...
      const targetState: any = navigationHistory[index]
  ...
  const handleDeleteFile = useCallback(
    (path: string) => {
      ...
      const isFolder = !schemasMap.has(path) && Array.from(schemasMap.keys()).some((key) => key.startsWith(path + "/"))
  ...
  const handleImportSchemas = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // Update summary
        const summary =
          `// Imported ${newSchemasMap.size} schemas from ZIP\n// Found ${allRefs.length} $ref relationships\n\n` +
          `Schemas:\n${Array.from(newSchemasMap.keys())
            .map((name) => `- ${name}`)
            .join("\n")}`
        setSchema(summary)
    ...
    async (e: React.DragEvent) => {
      ...
      try {
        ...
        if (addedCount > 0) {
          ...
          const summary =
            `// Loaded ${newSchemasMap.size} schemas (${addedCount} new)\n// Found ${allRefs.length} $ref relationships\n\n` +
            `Schemas:\n${Array.from(newSchemasMap.keys())
              .map((name) => `- ${name}`)
              .join("\n")}`
          setSchema(summary)
    ...
    >
      <header className="app-header">
        <div className="header-left">
          ...
          {navigationHistory.length > 0 && (
            <div className="breadcrumb-container">
              <button className="breadcrumb-item clickable" onClick={handleNavigateToRoot} title="Navigate to root">
                <Home size={14} />
              </button>
        ...
        <div className="actions">
          ...
          {schemasMap.size > 0 && (
            <button className="btn-export" onClick={handleExportSchemas} title="Export all schemas as ZIP">
              <Download size={16} />
              Export ZIP
            </button>
          )}
          {selectedNodeId && (
            <button className="btn-drill-down" onClick={handleDrillDown} title="Drill down into this schema">
              <ArrowDown size={16} />
              Drill Down
            </button>
          )}
      ...
      <main className="main-content">
        <div
          className={`editor-pane ${isEditorMinimized ? "minimized" : ""} ${viewMode === "multi" ? "multi-mode" : ""}`}
        >
          {!isEditorMinimized &&
              ...
              <JsonEditor
                ...
                onSwitchToMultiMode={() => {
                  ...
                  layoutGraph(rawNodes, rawEdges).then(({ nodes: layoutedNodes, edges: layoutedEdges }: any) => {
                    ...
                    const summary =
                      `// Loaded ${updatedMap.size} schemas\n// Found ${allRefs.length} $ref relationships\n\n` +
                      `Schemas:\n${Array.from(updatedMap.keys())
                        .map((name) => `- ${name}`)
                        .join("\n")}`
                    setSchema(summary)
          ...
          <div className="modal-content">
            ...
            <div className="url-input-container">
              ...
              <input
                ...
                className="url-input"
              />
              <small>Enter a URL to blob storage with the schemas</small>
