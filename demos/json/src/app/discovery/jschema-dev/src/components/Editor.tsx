            ...
            editor.onMouseDown((e) => {
                ...
                if (position) {
                    ...
                    if (model) {
                        ...
                        const jsonMatch: any = lineContent.match(/([a-zA-Z0-9._-]+\.json)/)
                        if (jsonMatch) {
...
export const MultiSchemaEditor: React.FC<MultiSchemaEditorProps> = ({
    ...
    useEffect(() => {
        ...
        const validTabs: any = savedState.openTabs.filter((tab) => schemasMap.has(tab))
    ...
    const handleTabClose = useCallback((e: React.MouseEvent, path: string) => {
        ...
        const newTabs: any = openTabs.filter((tab) => tab !== path)
        setOpenTabs(newTabs);
