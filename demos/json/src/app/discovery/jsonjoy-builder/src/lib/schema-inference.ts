import { asObjectSchema, type JSONSchema } from "../types/jsonSchema";
...
function detectEnumsInArrayItems(
  ...
  const valueMap: Record<string, Set<string | number>> | any = {};
  ...
  const updatedProperties: any = { ...mergedProperties };
...
function detectSemanticFormatsInArrayItems(
  ...
  for (const key in updatedProperties) {
    const currentSchema: any = asObjectSchema(updatedProperties[key])
...
function processArrayOfObjects(
  ...
  let mergedProperties: Record<string, JSONSchema> | any = {};
...
function inferArraySchema(obj: unknown[]): JSONSchema {
  ...
  const itemSchemas: any = obj.map((item) => inferSchema(item)) // Recursive call

  const firstItemSchema = asObjectSchema(itemSchemas[0]);
  const allSameType = itemSchemas.every((schema: any) => asObjectSchema(schema).type === firstItemSchema.type)
  ...
  // Mixed type arrays
  const uniqueSchemas: any = [...new Map(itemSchemas.map((s: any) => [JSON.stringify(s), s])).values()]
