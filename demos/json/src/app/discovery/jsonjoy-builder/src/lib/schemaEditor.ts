...
export function removeObjectProperty(
  ...
  const newSchema = copySchema(schema);
  const { [propertyName]: _, ...remainingProps }: any = newSchema.properties
...
export function createFieldSchema(field: NewField): JSONSchema {
  const { type, description, validation }: any = field
...
export function renameObjectProperty(
  ...
  const newSchema: any = copySchema(schema)
  const newProperties: Record<string, JSONSchema> | any = {};
  ...
  if (newSchema.required) {
    newSchema.required = newSchema.required.map((field: any) => (field === oldName ? newName : field))
  }
