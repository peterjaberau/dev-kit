...
const themeOptions: ThemeOption[] | any = [
...
export const Test = () => {
  ...
  const [selectedData, setSelectedData]: any = useState(testDataOptions[0])
  ...
  const groupedOptions = useMemo<Record<string, Array<TestDataOption & { index: number }>>>(() => {
    return testDataOptions.reduce(
      (acc: any, option: any, index) => {
  ...
  const handlePresetChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const option: any = testDataOptions[selectedIndex]
  ...
  const selectedTheme = useMemo(() => {
    return themeOptions.find((option: any) => option.id === selectedThemeId)?.theme ?? themeDefault
  }, [selectedThemeId])

  const selectedThemeLabel = useMemo(() => {
    return themeOptions.find((option: any) => option.id === selectedThemeId)?.label ?? "Default"
  }, [selectedThemeId])
            ...
            <section className="panel-section">
              ...
              {isCustomMode && (
                <label className="field">
                  ...
                  {parseError ? (
                    <p className="field-hint error">{parseError}</p>
                  ) : (
                    <p className="field-hint">Valid JSON preview.</p>
                  )}
                </label>
              )}
            </section>
            ...
            <section className="panel-section">
              ...
              <label className="field" aria-label="Expand level">
                ...
                <select
                  ...
                  onChange={(ev) =>
                    setExpandLevel(
                      isFinite(Number(ev.target.value)) ? Number(ev.target.value) : Boolean(ev.target.value == "true"),
                    )
                  }
                >
              ...
              <label className="field" aria-label="Theme selection">
                ...
                <select value={selectedThemeId} onChange={(ev) => setSelectedThemeId(ev.target.value)}>
        ...
        <section className="viewer">
          <div className="viewer-header">
            <div className="viewer-title">
              ...
              <p>
                Press <kbd>Cmd/Ctrl</kbd> + <kbd>F</kbd> to search. Use <kbd>Enter</kbd> / <kbd>Shift</kbd>+
                <kbd>Enter</kbd> to navigate results, <kbd>Esc</kbd> to close.
              </p>
            </div>
          ...
          >
            ...
            <SearchComponent
              ...
              scrollToPaths={async (paths, options) =>
                objViewRef?.current?.scrollToPaths(paths, options, 100 + paths.length * 14, 50)
              }
              ref={searchRef}
            />
          </div>
