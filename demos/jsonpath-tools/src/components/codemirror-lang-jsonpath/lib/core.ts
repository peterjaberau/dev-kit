import { Diagnostics, QueryOptions, TextChange, TextRange } from "#components/jsonpath";
...
import { DataType } from "#components/jsonpath";
...
export const languageServiceSessionStateField = StateField.define<LanguageServiceSession>({
    create(state) {
        const languageServices: any = state.facet(languageServiceFacet);
