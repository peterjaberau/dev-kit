...
export class ObjectDataType extends DataType {
    ...
    changeTypeAtPath(path: NormalizedPath | any, operation: (currentType: DataType) => DataType): DataType {
        if (path.length === 0)
    ...
    setPathExistence(path: NormalizedPath | any): DataType {
        if (path.length === 0)
            return this;
...
export class ArrayDataType extends DataType {
    ...
    changeTypeAtPath(path: NormalizedPath | any, operation: (currentType: DataType) => DataType): DataType {
        ...
        const newPrefixElementTypes: any = [...this.prefixElementTypes];
    ...
    static create(types: readonly DataType[], annotations = DataTypeAnnotation.EMPTY_SET): DataType {
        ...
        for (let i = 0; i < flattenedTypes.length; i++) {
            for (let j = i + 1; j < flattenedTypes.length; j++) {
                const typeI: any = flattenedTypes[i];
                const typeJ: any = flattenedTypes[j];
        ...
        const filteredFlattenedTypes: any = flattenedTypes.filter(type => !(type instanceof NeverDataType));
