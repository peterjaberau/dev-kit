...
export class FormattingService {
    ...
    private formatTree(tree: SyntaxTree, baseIndent: number, context: FormatterContext) {
        if (tree instanceof SyntaxTreeNode) {
            ...
            for (let i = 0; i < tree.children.length; i++) {
                const child: any = tree.children[i];
                let wasIndent = false;
                if (i !== 0) {
    ...
    private addEdit(tree: SyntaxTree, baseIndent: number, whitespacePolicy: WhitespacePolicy, context: FormatterContext) {
        const lines: any = this.getSkippedTextLines(tree);
    ...
    private adjustSpaces(text: string | any, textPosition: number, textOffset: number, textLength: number, character: string, count: number, context: FormatterContext) {
