...
export class Parser {
    ...
    private parseBracketedSelection(context: ParserContext, dotToken: SyntaxTreeToken | null, type: SegmentType): Segment {
        ...
        const selectors: { selector: Selector, commaToken: SyntaxTreeToken | null }[] | any = [];
        this.skipWhitespace(context);
    ...
    private parseOrExpression(context: ParserContext): FilterExpression {
        const andExpressions: { expression: FilterExpression, orToken: SyntaxTreeToken | null }[] | any = [];
        ...
        andExpressions.forEach((e: any) => { if (e.expression !== null) this.checkLogicalExpressionOperand(e.expression, context); });
        return new OrExpression(andExpressions);
    }

    private parseAndExpression(context: ParserContext): FilterExpression {
        const basicExpressions: { expression: FilterExpression, andToken: SyntaxTreeToken | null }[] | any = [];
        ...
        basicExpressions.forEach((e: any) => { if (e.expression !== null) this.checkLogicalExpressionOperand(e.expression, context); });
    ...
    private parseFunctionArguments(context: ParserContext): { arg: FilterExpression, commaToken: SyntaxTreeToken | null }[] {
        const args: { arg: FilterExpression, commaToken: SyntaxTreeToken | null }[] | any = [];
    ...
    private checkFunctionName(nameToken: SyntaxTreeToken, context: ParserContext) {
        if (!CharacterCategorizer.isFunctionNameFirst((nameToken as any).text[0]))
...
class ParserContext {
    ...
    get current(): string | null {
        return this._currentIndex < this.queryText.length
            ? (this as any).queryText[this._currentIndex]
            : null;
    }

    get next(): string | null {
        return this._currentIndex + 1 < this.queryText.length
            ? (this as any).queryText[this._currentIndex + 1]
            : null;
    }
