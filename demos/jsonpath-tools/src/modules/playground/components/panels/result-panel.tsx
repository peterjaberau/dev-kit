...
import { InputWrapper, Modal, SegmentedControl, Select } from "@mantine/core"
import { IconButton, Button, Group, Stack } from "@chakra-ui/react"
        ...
        toolbar={
          <>
            ...
            <IconButton
              ...
              onClick={async () => await saveTextFile("result.json", "application/json", ".json", resultText)}
            >
              <IconFileDownload />
            </IconButton>
          </>
...
function ReplacementEditor({
  ...
  const form = useForm({
    ...
    validate: {
      jsonValueText: (value, values) =>
        values.type === OperationReplacementType.jsonValue ? isJSONString(value) : null,
      jsonPatchText: (value, values) =>
        values.type === OperationReplacementType.jsonPatch ? isJSONPatchString(value) : null,
    },
  })
        ...
        {type === OperationReplacementType.jsonValue && (
          <EditorFormAdapter
            editor={(value, onValueChange, onFocus, onBlur) => (
              <JSONEditor value={value} onValueChanged={onValueChange} onFocus={onFocus} onBlur={onBlur} />
            )}
            label="JSON Value"
        ...
        {type === OperationReplacementType.jsonPatch && (
          <EditorFormAdapter
            ...
            description={
              <>
                JSON Patch (
                <a href="https://datatracker.ietf.org/doc/html/rfc6902" target="_blank">
                  RFC 6902
                </a>
                ) document that will be applied to every result node in the input JSON (post-order, i.e. descendants
                first).
              </>
            }
