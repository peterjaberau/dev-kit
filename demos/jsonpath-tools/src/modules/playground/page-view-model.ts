import { Parser } from "#components/jsonpath";
import { Query } from "#components/jsonpath";
import { removeAtPaths, replaceAtPaths } from "#components/jsonpath";
import { serializedNormalizedPath } from "#components/jsonpath";
import { NormalizedPath } from "#components/jsonpath";
import { Nothing } from "#components/jsonpath";
import { JSONValue } from "#components/jsonpath";
import { useState, useMemo, useRef, useEffect, useCallback } from "react";
import { OperationCancelledError } from "#components/codemirror-lang-jsonpath";
...
let worker: Worker | undefined | any;
let languageService: LanguageService | undefined | any;
if (typeof window !== "undefined") {
  worker = new Worker(new URL("./services/language-service/custom-language-service-worker.ts", import.meta.url), { type: "module" });
  languageService = new LanguageService(data => worker!.postMessage({ type: "languageServiceData", data } as CustomLanguageServiceWorkerMessage));
