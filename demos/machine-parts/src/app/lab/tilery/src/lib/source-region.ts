...
function readSourceRegionMarker(
  ...
  const lineCommentMatch: any = trimmed.match(
  ...
  const jsxCommentMatch: any = trimmed.match(
...
function dedentSource(source: string): string {
  ...
  const indent = Math.min(
    ...lines.filter((line: any) => line.trim().length > 0).map((line: any) => line.match(/^\s*/)?.[0].length ?? 0),
  )

  if (!Number.isFinite(indent) || indent === 0) return lines.join('\n');
  return lines.map((line: any) => line.slice(indent)).join("\n")
}

function trimBlankLines(lines: string[] | any) {
  let start = 0;
