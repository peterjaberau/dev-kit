...
import { Graph, topologicalSort, willCreateCycle } from "../../utils/graphology"
...
export function resolveRulesExtendedViews<A extends AnyAux, V extends Record<any, LikeC4View<A>>>(
  ...
  return sorted.reduce(
    (acc: any, id: any) => {
      const view: any = g.getNodeAttribute(id, "view")
      if (!isExtendsElementView(view)) {
        //@ts-ignore
        acc[view.id] = view
        return acc
      ...
      const tags = unique([...(extendsFrom.tags ?? []), ...(view.tags ?? [])]) as aux.Tags<A>
