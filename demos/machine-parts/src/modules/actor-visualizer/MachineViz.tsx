      ...
      <MachineBody>
        {rootEdges.length > 0 && (
          <MachineTransitions>
            {rootEdges.map((edge, i) => (
              <TransitionVisual key={edge.id} edge={edge} graph={graph} sourceId={root.id} isFirst={i === 0} />
            ))}
          </MachineTransitions>
        )}
        ...
        <MachineStateList>
          {sortedStates.map((child) => (
            <StateNodeVisual key={child.id} node={child} graph={graph} isInitial={root.data.initialId === child.id} />
          ))}
        </MachineStateList>
      </MachineBody>
    </MachineRoot>
