...
function LogViewerTerminal({
  ...
  const { scrollRef, isAtBottom, handleScroll, scrollToBottom } = useAutoScroll(entries, autoScroll && !paused)
  ...
  return (
    <div
      ...
      className={cn("border-border/60 bg-card flex flex-col overflow-hidden rounded-xl border shadow-sm", className)}
      {...props}
    >
      {/* Toolbar */}
      ...
      <div
        ...
        className="bg-card overflow-auto font-mono text-xs leading-relaxed [scrollbar-width:thin]"
        style={{ maxHeight }}
        role="log"
        aria-live="polite"
        aria-label={title}
          ...
          filteredEntries.map((entry, i) => {
            ...
            return (
              ...
              >
                {lineNumbers && (
                  <span
                    className="text-muted-foreground/50 shrink-0 select-none text-right"
                ...
                {timestamps && (
                  <span className="text-muted-foreground/60 shrink-0">{formatTimestampFull(entry.timestamp)}</span>
                )}
                <span className={cn("w-[3ch] shrink-0 text-right font-semibold", colors.text)}>
                  {LEVEL_LABELS[entry.level]}
                </span>
                <span className="text-foreground/90 min-w-0 flex-1 whitespace-pre-wrap break-all">
...
function LogViewerMinimal({
  ...
  const { scrollRef, isAtBottom, handleScroll, scrollToBottom } = useAutoScroll(entries, autoScroll)

  return (
    <div
      data-slot="log-viewer-minimal"
      className={cn("border-border/60 bg-card flex flex-col overflow-hidden rounded-lg border shadow-sm", className)}
      {...props}
    >
      <div
        ref={scrollRef}
      ...
      >
        {entries.length === 0 ? (
          <div className="text-muted-foreground flex items-center justify-center py-8 text-sm">No log entries.</div>
        ) : (
          entries.map((entry, i) => {
            const colors = resolveLevelColors(entry.level, colorScale)
            return (
              <div key={i} className="border-border/20 flex items-start gap-2 border-b px-3 py-1.5">
                <Circle className={cn("mt-[3px] size-2 shrink-0 fill-current", colors.text)} aria-label={entry.level} />
                {timestamps && (
                  <span className="text-muted-foreground/60 shrink-0">{formatTimestamp(entry.timestamp)}</span>
                )}
                <span className="text-foreground/90 min-w-0 flex-1 whitespace-pre-wrap break-all">{entry.message}</span>
              </div>
            )
          })
        )}
  ...
  return (
    ...
    >
      ...
      <div className="border-border/40 bg-muted/30 flex items-center gap-2 border-b px-3 py-2">
        ...
        <div className="flex items-center gap-0.5">
          <ToolbarButton onClick={handleCopyFiltered} label={copied ? "Copied" : "Copy filtered logs"}>
            {copied ? <Check className="size-3.5 text-emerald-500" /> : <Copy className="size-3.5" />}
          </ToolbarButton>
        ...
        <div className="flex items-center gap-1">
          {levels.map((level) => {
            ...
            return (
              ...
              >
                <Circle className={cn("size-1.5 fill-current", isActive ? colors.text : "text-muted-foreground/30")} />
                {LEVEL_LABELS[level]}
        ...
        {filteredEntries.length === 0 ? (
          ...
          filteredEntries.map((entry, i) => {
            ...
            return (
              ...
              >
                <Circle className={cn("mt-[5px] size-2 shrink-0 fill-current", colors.text)} aria-hidden="true" />
                {timestamps && (
                  <span className="text-muted-foreground/60 shrink-0">{formatTimestamp(entry.timestamp)}</span>
                )}
                <span className={cn("w-[3ch] shrink-0 text-right font-semibold", colors.text)}>
