...
import { useDockview, useInteractions } from "#adaptive-view/sandbox/desktop/selectors"

export function DebuggerGroupsView() {
  const { groupsList } = useDockview()
...
function GroupCard({ id, name }: { id: string; name: string }) {
  const { selectedGroupId, sendToDesktop } = useInteractions()
