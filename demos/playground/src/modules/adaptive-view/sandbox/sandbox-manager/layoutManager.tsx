...
import {
    ...
    generateLayoutManagerCodeSnippet,
...
const TextRow = (props: {
    label: string;
    varName?: keyof LayoutManagerThemeCssOverrides;
    value: string;
...
type LayoutManagerTab = 'theme' | 'controls';

// THEME / CONTROLS as a full-width segmented pill (active segment = accent fill).
const TabToggle = (props: {
    active: LayoutManagerTab;
    onChange: (tab: LayoutManagerTab) => void;
}) => (
...
export const LayoutManager = (props: {
    ...
    onChange: (patch: Partial<LayoutManagerThemeState>) => void;
    onCssChange: (patch: Partial<LayoutManagerThemeCssOverrides>) => void;
    onReset: () => void;
    baseTheme: DockviewTheme;
    containerEl: HTMLElement | null;
    controls?: React.ReactNode;
}) => {
    const [activeTab, setActiveTab] = React.useState<LayoutManagerTab>('theme');
    ...
    const set = (patch: Partial<LayoutManagerThemeCssOverrides>) => props.onCssChange(patch);

    const computedVar = (varName: string, fallback: number): number => {
        const override = css[varName as keyof LayoutManagerThemeCssOverrides];
    ...
    const code = generateLayoutManagerCodeSnippet(props.baseTheme, props.state);
    ...
    const colorRow = (label: string, varName: keyof LayoutManagerThemeCssOverrides) => (
        <ColorRow
            key={varName}
            label={label}
            varName={varName}
            value={css[varName] ?? ''}
            onChange={(v) =>
                set({ [varName]: v || undefined } as Partial<LayoutManagerThemeCssOverrides>)
            }
    ...
    return (
        <div
            className="dv-layout-manager-panel"
            style={{
                width: '332px',
                background: LM.bg,
                color: LM.text,
