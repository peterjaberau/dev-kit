...
export type ViewPanelZone = ViewDirection | "center"
 ...
 */
export function viewEdgeZoneAt(
  rect: { left: number; top: number; width: number; height: number },
  x: number,
  y: number,
  edgeSize: number = 24,
 ...
 */
export type ViewTabBarHit = { kind: "before"; tabId: string } | { kind: "after"; tabId: string } | { kind: "append" }
 ...
 */
export function viewTabBarDropAt(
  ...
  for (const tr of tabRects) {
    ...
    if (x <= tr.right) {
      ...
      return x < mid ? { kind: "before", tabId: tr.tabId } : { kind: "after", tabId: tr.tabId }
    }
  }
  return { kind: "append" }
}
