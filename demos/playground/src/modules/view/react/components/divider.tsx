...
import type { ViewDivider as DividerType, ViewSizeResolutionContext } from "../../core/internal"
import { viewRectEdgePercent } from "../../core/internal"
import { useViewPointerDrag } from "../use-pointer-drag"

/**
 * Props for the {@link ViewDivider} component.
 */
export type DividerProps = {
   ...
   */
  onReset?: (dividerId: string, sizeContext?: ViewSizeResolutionContext) => boolean | void
 ...
 */
export const ViewDivider = memo(function ViewDivider({
  divider,
  accessibility,
  disabled = false,
  ...
  const onMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const pct = viewRectEdgePercent(rect, e.clientX, e.clientY, divider.orientation === "vertical" ? "left" : "top")
  ...
  const handlers = useViewPointerDrag({ onMove })
  const minPosition = Math.min(accessibility.minPosition, accessibility.maxPosition)
  const maxPosition = Math.max(accessibility.minPosition, accessibility.maxPosition)
  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      if (disabled) return
      const min = Math.min(accessibility.minPosition, accessibility.maxPosition)
      const max = Math.max(accessibility.minPosition, accessibility.maxPosition)
      const step =
  ...
  return (
    <div
      ...
      aria-keyshortcuts={
        divider.orientation === "vertical" ? "ArrowLeft ArrowRight Home End" : "ArrowUp ArrowDown Home End"
      }
    />
  )
})
