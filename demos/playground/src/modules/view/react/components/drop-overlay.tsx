...
import type { ViewDragState } from "../../core/internal"
...
function renderRootZone(
  ...
  if (!zone) return null;
  const mainLayer = container.querySelector<HTMLElement>('.view__main-layer');
...
function renderTabBarInsertion(
  hoverTabBar: NonNullable<ViewDragState['hoverTabBar']>,
  container: HTMLElement,
  cRect: DOMRect,
): React.ReactNode {
  const { panelId, hit } = hoverTabBar;
  const tabBar = container.querySelector<HTMLElement>(
    `.view__panel[data-panel-id="${cssEscape(panelId)}"] .view__tab-list, .view__panel[data-panel-id="${cssEscape(panelId)}"] .view__tab-bar`,
  );
  ...
  return (
    <div
      className="view__drop-insertion"
      aria-hidden="true"
      style={{
        left: x - cRect.left,
...
function renderPanelZone(
  panelEl: HTMLElement,
  zone: NonNullable<ViewDragState['hoverZone']>,
  cRect: DOMRect,
): React.ReactNode {
  const panelRect = panelEl.getBoundingClientRect();
  const tabBar = panelEl.querySelector<HTMLElement>('.view__tab-bar');
