...
import { viewPanelDomId } from '../dom-ids';
...
const floatingResizeEdges: ViewFloatingResizeEdge[] = [
  'top',
  'right',
  'bottom',
  'left',
...
export const PanelChrome = memo(function PanelChrome({
  ...
  const behavior = viewPanelBehaviorFromState(view.getState(), panel.id);
  ...
  return (
    <div
      id={viewPanelDomId(panel.id)}
      ref={registerPanel}
      className="view__panel"
      ...
      <TabBar
        ...
        {...tabBarProps}
      />
      <div ref={registerContentSlot} className="view__panel-content" />
      {isFloating && !isNativePopout && !isFullScreen && behavior.resizable
        ? floatingResizeEdges.map((edge) => (
            <div
              key={edge}
              className="view__floating-resize-handle"
