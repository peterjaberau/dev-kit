...
import type { ViewController, ViewPanel, ViewTab } from '../../core/internal';
import { viewPanelBehaviorFromState } from '../../core/internal';
import { PanelActions, type PanelActionsProps } from './panel-actions';
import { Tab } from './tab';
import type { ViewTabTriggerRenderer } from '../view';
import { viewCssEscape as cssEscape } from '../css-escape';
...
export type TabBarProps = {
   ...
   * children.
   */
  renderTabTrigger?: ViewTabTriggerRenderer;
...
export function TabBar({
  ...
  const overflowMenu = useViewMenu();
  const behavior = viewPanelBehaviorFromState(view.getState(), panelId);
  const canDragPanel =
    panel.kind !== 'edge' &&
    behavior.draggable &&
    panel.tabs.length > 0 &&
  ...
  const handleBarDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (
        ...
        !target.closest('[data-view-tab-overflow]') &&
        !target.closest('[data-view-panel-actions]')
      ) {
  ...
  return (
    <div
      ...
      {hasHiddenTabs && (
        <div
          className="view__tab-overflow"
          data-view-tab-overflow=""
          onPointerDown={(e) => {
            e.stopPropagation();
          }}>
          <button
            ...
            type="button"
            className="view__panel-action-button view__tab-overflow-button"
          ...
          {overflowMenu.isOpen && (
            <div
              ...
              className="view__panel-menu view__tab-overflow-menu"
              role="menu"
              ...
              {hiddenTabs.map((tab) => {
                ...
                return (
                  <button
                    key={tab.id}
                    type="button"
                    className="view__panel-menu-item view__tab-overflow-menu-item"
...
function getHiddenTabIds(tabList: HTMLElement, tabs: readonly ViewTab[]) {
