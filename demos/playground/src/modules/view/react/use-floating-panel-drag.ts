...
import {
  ...
  type ViewFloatingPanelBounds,
  type ViewFloatingResizeEdge,
  type ViewController,
  type ViewPanelId,
} from '../core/internal';

type ViewFloatingPanelDragState = {
  panelId: ViewPanelId;
  pointerId: number;
  edge?: ViewFloatingResizeEdge;
  startX: number;
  startY: number;
  startBounds: ViewFloatingPanelBounds;
  containerWidth: number;
  containerHeight: number;
};

/**
 * Pointer event handlers returned by {@link useViewFloatingPanelDrag} that
 * drive both move and resize interactions for floating panels.
 */
export type ViewFloatingPanelDragController = {
 ...
 * updating panel bounds live via the View controller on every pointer-move.
 */
export function useViewFloatingPanelDrag({
  view,
  containerRef,
  onTabPointerMove,
  onTabBarPointerUp,
...
}): ViewFloatingPanelDragController {
  const floatingDragRef = useRef<ViewFloatingPanelDragState | null>(null);
  ...
  const onFloatingTabBarPointerDown = useCallback(
    (e: PointerEvent, panelId: string) => {
      ...
      panel.focus();
      const behavior = viewPanelBehaviorFromState(m.getState(), panelId);
  ...
  const onFloatingResizePointerDown = useCallback(
    (e: PointerEvent, panelId: string, edge: ViewFloatingResizeEdge) => {
      if (e.button !== 0) return;
      const m = view();
      const panel = m?.getPanel(panelId);
      const bounds = panel?.floatingBounds;
  ...
  const onPointerMove = useCallback(
    (e: PointerEvent) => {
      ...
      view()?.setFloatingPanelBounds(floatingDrag.panelId, nextBounds);
    },
    [onTabPointerMove, view],
  );
