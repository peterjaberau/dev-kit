...
import { RelationGraphJsxProps } from "../types"
import { deprecatedWaring, devLog, relationGraphVersionInfo } from "#models-core/utils/RGCommon"
import { getEventListeners } from "#models-core/utils/RGIntergration"
...
const Index: React.ForwardRefRenderFunction<RelationGraphExpose, RelationGraphJsxProps> = (props, ref) => {
  ...
  const theInstance = useRef(null) as RefObject<RelationGraphInstance> | any
  useEffect(() => {
  ...
  useImperativeHandle(ref, (): RelationGraphExpose => {
    return {
      ...
      async setJsonData(
        ...
        await theInstance.current.setJsonData(jsonData, true)
  ...
  if (!theInstance.current) {
    ...
    const newInstance = props.relationGraphCore
      ? new props.relationGraphCore(props.options, getEventListeners(props))
      : new RelationGraphFinal(props.options, getEventListeners(props))
  ...
  const slotsOnCanvasBehind = React.Children.toArray(props.children).filter((child: React.ReactNode | any) => {
    if (child) {
