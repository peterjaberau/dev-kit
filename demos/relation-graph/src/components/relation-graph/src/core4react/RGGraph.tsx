...
import type { RGLineSlotProps, RGNodeSlotProps } from '#models-core/types';
...
const RGSingleGraph: React.FC<RGSingleGraphProps> = (canvasProps) => {
  ...
  const rgCanvas$ = useRef(null as any) as RefObject<HTMLDivElement>;
  useEffect(() => {
    init();
  }, []);
  const init = () => {
  ...
  const LineSlot: any = canvasProps.lineSlot;
  return (
    <>
