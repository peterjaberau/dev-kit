...
import RGNodesAnalytic from '#models-core/utils/RGNodesAnalytic';
import { RGUpdateContext, RelationGraphStoreContext } from "#components";
import RGNodeExpandHolder from './RGNodeExpandHolder';
import {RGNodeProps} from '#components/types';

const SeeksRGNode: React.FC<RGNodeProps> = ({nodeProps, NodeSlot, ExpandHolderSlot}: any) => {
  const relationGraph = useContext(RelationGraphStoreContext);
  const updateView = useContext(RGUpdateContext);
  const seeksRGNodeRef = useRef(null as any) as RefObject<HTMLDivElement>;
  useEffect(() => {
