...
export class SeeksFolderLayouter extends SeeksBaseLayouter {
  constructor(layoutOptions: RGTreeLayoutOptions, graphOptions: RGOptionsFull, graphInstance: RelationGraphFinal) {
    ...
    if (this.layoutOptions.levelDistance) {
      if (typeof this.layoutOptions.levelDistance === "string") {
        this.levelDistanceArr = this.layoutOptions.levelDistance.split(",").map((thisNum: any) => Number.parseInt(thisNum))
      } else if (Array.isArray(this.layoutOptions.levelDistance)) {
        this.levelDistanceArr = this.layoutOptions.levelDistance
      }
    }
    //@ts-ignore
    this.enableGatherNodes = (this.layoutOptions as any).enableGatherNodes
    // this.layoutOptions.layoutExpansionDirection; // 节点扩展方向[left/top]/center/[right/bottom]
  }
  enableGatherNodes: any = false
  layoutOptions: RGTreeLayoutOptions | any
  ...
  analysisNodes4Didirectional(
    ...
    if (newLevelNodes.length > 0) {
      ...
      willLayoutNodes.forEach((thisNode: any) => {
        thisNode.lot.strengthWithChilds = 1
        RGNodesAnalytic.conductStrengthToParents4Folder(thisNode)
      })
      RGNodesAnalytic.analysisDataFolder([willLayoutNodes[0]], 0, levelDirect)
  ...
  placeNodesPosition(rootNode: RGNode | any, groupNodes: RGNode[], analyticResult: NodesAnalyticResult) {
  ...
  placeRelativePosition(rootNode: RGNode, groupNodes: RGNode[], analyticResult: NodesAnalyticResult) {
    const __per_width = getNumberOption(this.layoutOptions.min_per_width) || 50
    const __per_height = getNumberOption(this.layoutOptions.min_per_height) || 40
  ...
  gatherNodes(groupNodes: RGNode[], hv: "h" | "v", perSize: number) {
    ...
    groupNodes.forEach((thisNode: any) => {
    ...
    groupNodes.forEach((thisNode: any) => {
      ...
      if (thisNode.lot.level !== 0) {
        const levelId = thisNode.lot.level + ""
        const levelNodes: any = levelMapping[levelId]
        if (thisNode.lot.strengthWithChilds === 1) {
          if (thisNode.lot.childs_size! <= 1) {
            // thisNode.selected = true;
            //@ts-ignore
            const bloomingNode: any = this.getBloomingNearByParent(thisNode, thisNode.lot.parent!, levelNodes, hv)
            // console.log(thisNode.text, 'bloomingNode:', bloomingNode && bloomingNode.text, bloomingNode && bloomingNode.lot.y, perSize);
            if (bloomingNode) {
              if (hv === "h") {
                if ((((thisNode.lot.y as any) - bloomingNode.lot.y) as any) > 0) {
  ...
  getBloomingNearByParent(
    node: RGNode,
    parentNode: RGNode | any,
    levelNodes: RGNode[],
    hv: "h" | "v",
  ): RGNode | undefined {
    let minDistance = 9999
    let minNode: RGNode | undefined
    for (const thisNode of levelNodes as any) {
  ...
  getLevelDistance(node: RGNode, level: number, perSize: number) {
    ...
    if (this.levelDistanceArr && this.levelDistanceArr.length > 0) {
      ...
      for (let i = 0; i < absLevel; i++) {
        const _thisLevelDistance: any =
          i >= this.levelDistanceArr.length
            ? this.levelDistanceArr[this.levelDistanceArr.length - 1]
            : this.levelDistanceArr[i]
