...
export class RelationGraphWith2Data extends RelationGraphWith1Dom {
  ...
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  reactiveData:RGGraphReactiveData | any;
   ...
   * @param reactiveData
   */
  setReactiveData(graphData:RGGraphData | any, reactiveData:RGGraphReactiveData | any) {
   ...
   * @param reactiveData
   */
  setReactiveDataVue3(graphData:RGGraphData | any, reactiveData:RGGraphReactiveData | any) {
    this.reactiveData = reactiveData;
    this.graphData = graphData;
    this.allLineColors = reactiveData.allLineColors;
  ...
  protected _initLayoutByLayoutOptions(layoutOptions:RGLayoutOptions) {
    ...
    this.layouter = createLayout(layoutOptions, this.options, this as any);
  }
  protected initLayouter() {
    if (this.userLayouerClass) {
      ...
      if (this.options.layouts && this.options.layouts.length > 0) {
        const _defaultLayoutSetting: any = this.options.layouts[0];
