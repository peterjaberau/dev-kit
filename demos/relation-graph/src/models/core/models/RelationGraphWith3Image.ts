...
export class RelationGraphWith3Image extends RelationGraphWith2Data {
  ...
  private mergeCanvas(
    canvasBg: HTMLCanvasElement | null,
    canvasMain: HTMLCanvasElement,
    canvasWatermark: HTMLCanvasElement | null,
  ) {
    ...
    if (canvasBg) {
      ...
      ctx.fillStyle =
        !this.options.backgroundColor || this.options.backgroundColor === "transparent"
          ? "#ffffff"
  ...
  private async createGraphMainCanvas(format = "png") {
    ...
    for (const canvasSlot of canvasSlotList) {
      ...
      if (sub_min_x !== 999999)
        canvasSlotInfoMap.set(canvasSlot, {
          offsetX: sub_min_x,
          offsetY: sub_min_y,
        })
    }
  ...
  async createImage(exportDom: HTMLDivElement, opts: any, format: string, fileName: string) {
