...
export class RelationGraphWith4Line extends RelationGraphWith3Image {
  ...
  private _getJunctionPoint(
    junctionPointStyle: RGJunctionPoint | any,
    createJunctionPointParams: CreateJunctionPointParams | any,
  ) {
    if (junctionPointStyle === JUNCTION_POINT_STYLE.border) {
      ...
      //@ts-ignore
      return RGGraphMath.getRectJoinPoint(...(createJunctionPointParams as any))
  ...
  createLinePathByTwoNode(
    ...
    let __lineShape = relationData.lineShape || this.options.defaultLineShape || 1
    ...
    const fromNodeShape =
      viewFromNode.nodeShape !== undefined && viewFromNode.nodeShape !== null
        ? viewFromNode.nodeShape
    ...
    const toNodeShape =
      viewToNode.nodeShape !== undefined && viewToNode.nodeShape !== null
        ? viewToNode.nodeShape
        : this.options.defaultNodeShape
    ...
    const defaultJunctionPointStyle: RGJunctionPoint = (this.options.defaultJunctionPoint ||
      JUNCTION_POINT_STYLE.border) as RGJunctionPoint

    let fromJunctionPoint: RGJunctionPoint =
      relationData.fromJunctionPoint || viewFromNode.junctionPoint || defaultJunctionPointStyle
    let toJunctionPoint: RGJunctionPoint =
      relationData.toJunctionPoint || viewToNode.junctionPoint || defaultJunctionPointStyle
    ...
    if (_from === _to) {
      if (
        fromJuctionPointOffsetX === 0 &&
        fromJuctionPointOffsetY === 0 &&
        toJuctionPointOffsetX === 0 &&
        toJuctionPointOffsetY === 0
      ) {
        if (
          __lineShape === 1 ||
          __lineShape === 2 ||
  ...
  createLinePathData(
    ...
    if (lineShape === 4) {
      ...
      return this.createLineForCurve6(
        relation,
        textPosition,
        allLineSize,
        indexOfAllLine,
      ...
      return this.createLineForCurve(
        relation,
        textPosition,
        allLineSize,
        indexOfAllLine,
  ...
  createCheckedLinePath() {
    ...
    try {
      const elementLine: any = this.getElementLineById(this.options.checkedLinkId as string)
      if (elementLine) {
        const { path } = this.createLinePath(elementLine, elementLine.relations[0], 0)
        return path
      }
      const link: RGLink | any = this.getLinkByLineId(checkedLineId)
      if (!link) {
      ...
      const lineIndex: number = link.relations.findIndex((l: any) => l.id === checkedLineId)
      if (lineIndex === -1) {
        devLog("Can not find checkedLineId:", checkedLineId)
        return
      }
      const line: RGLine | any = link.relations[lineIndex]
      const { path } = this.createLinePath(link, line, lineIndex)
      return path
    } catch (e) {
      devLog(e)
    }
  ...
  createCheckedLineStrokeWidth() {
    const checkedLineId = this.options.checkedLineId
    if (!checkedLineId) {
      return (this.options.defaultLineWidth as any) + 8
    }
    try {
      const link: RGLink | any = this.getLinkByLineId(checkedLineId)
      if (!link) {
  ...
  protected createLineFor44(
    ...
    const pathInfo = createLine44PathData(
      relation,
      textPosition,
      allLineSize,
      indexOfAllLine,
