...
import { StreamResourcesEvent } from "../types";
...
//location
export const getLocationProjectTreeNodeData = (location: DragLocationHistory | any): DropNode | null => {
...
export const getLocationProjectTreeData = (location: DragLocationHistory | any): DropNode | null => {
  if (location.current.dropTargets.length === 0) return null;
...
export const getAllNestedLocationProjectTreeData = (location: DragLocationHistory | any): DropNode[] => {
  if (location.current.dropTargets.length === 0) return [];
  return location.current.dropTargets.map((target: any) => {
    return {
...
export const getLocationProjectTreeRootNodeData = (location: DragLocationHistory | any): DropRootNode | null => {
  if (location.current.dropTargets.length === 0) return null;
  if (location.current.dropTargets[0].data.type !== ProjectDragType.ROOT_NODE) return null;
...
export const getInstructionFromLocation = (location: DragLocationHistory | any): Instruction | null => {
  return extractInstruction(location.current.dropTargets[0].data);
};

//other checks
export const doesLocationHasProjectTreeNode = (location: DragLocationHistory | any): boolean => {
