...
import { dropTargetForElements, type ElementDropTargetEventBasePayload, monitorForElements } from "@atlaskit/pragmatic-drag-and-drop/element/adapter"
...
function createTreeItemRegistry() {
  ...
  const registerTreeItem = ({ itemId, element, actionMenuTrigger }: { itemId: string; element: HTMLElement; actionMenuTrigger: HTMLElement }): CleanupFn => {
...
export function Tree() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (lastAction.type === "modal-move") {
        ...
        liveRegion.announce(`You've moved Item ${lastAction.itemId} to position ${lastAction.index + 1} in ${parentName}.`)
  ...
  const context = useMemo<TreeContextValue>(
    () => ({
      ...
      getPathToItem: memoizeOne((targetId: string) => tree.getPathToItem({ current: lastStateRef.current, targetId }) ?? []),
      getMoveTargets,
      getChildrenOfItem,
      registerTreeItem,
    }),
  ...
  useEffect(() => {
    ...
    return combine(
      monitorForElements({
        canMonitor: ({ source }) => source.data.uniqueContextId === context.uniqueContextId && source.data.type === "tree-item",
        onDrop(args) {
      ...
      dropTargetForElements({
        ...
        canDrop: ({ source }) => source.data.uniqueContextId === context.uniqueContextId && source.data.type === "tree-item",
        getData: () => ({ type: "group" }),
  ...
  return (
    <TreeContext.Provider value={context}>
      ...
      <div style={{ display: "flex", justifyContent: "center", padding: 24 }}>
        <chakra.div css={treeStyles} id="tree" ref={ref}>
