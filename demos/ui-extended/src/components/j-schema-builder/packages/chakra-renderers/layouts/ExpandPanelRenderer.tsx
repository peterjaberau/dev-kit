...
import { JsonFormsDispatch, JsonFormsStateContext, withJsonFormsContext } from "#jSchemaBuilder/react"
import {
  ArrayTranslations,
  composePaths,
  ControlElement,
...
import { Accordion, Avatar, ButtonGroup, Flex, IconButton, Text } from "@chakra-ui/react"
...
const ExpandPanelRenderer = (props: ExpandPanelProps) => {
  ...
  const foundUISchema = useMemo(
    () => findUISchema(uischemas as JsonFormsUISchemaRegistryEntry[], schema, uischema.scope, path, undefined, uischema, rootSchema),
    ...
    return (
      <ButtonGroup>
        <Tooltip key="1" content="Move up">
          <IconButton aria-label={translations.upAriaLabel || ""} onClick={moveUp(path, index)} disabled={!enableMoveUp}>
            <LuArrowUp />
          </IconButton>
        </Tooltip>
        <Tooltip key="2" content="Move down">
          <IconButton aria-label={translations.downAriaLabel || ""} onClick={moveDown(path, index)} disabled={!enableMoveDown}>
            <LuArrowDown />
          </IconButton>
        </Tooltip>
        {appliedUiSchemaOptions.showSortButtons ? (
          <Tooltip key="3" content="Delete">
            <IconButton aria-label={translations.removeAriaLabel || ""} onClick={removeItems(path, [index])}>
              <LuTrash2 />
            </IconButton>
          </Tooltip>
        ) : null}
  ...
  return (
    <>
      <Accordion.Item key={key} value={key as any}>
        <Accordion.ItemTrigger>
          <Flex justifyContent="space-between" width="100%">
            <Flex alignItems="center">
              <Avatar.Root as="h4" size="sm" me="2" color="gray.800" bgColor="gray.100">
 ...
 */
export const ctxDispatchToExpandPanelProps: (dispatch: Dispatch<any>) => DispatchPropsOfExpandPanel = (dispatch) => ({
  removeItems:
 ...
 */
export const withContextToExpandPanelProps = (Component: any) =>
  function WithContextToExpandPanelProps({ ctx, props }: JsonFormsStateContext & ExpandPanelProps) {
...
export const withJsonFormsExpandPanelProps = (Component: any) => withJsonFormsContext(withContextToExpandPanelProps(Component) as any)
