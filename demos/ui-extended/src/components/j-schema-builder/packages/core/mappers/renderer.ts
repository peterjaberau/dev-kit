...
const move = (array: any[], index: number, delta: number) => {
  ...
  const indexes: number[] | any[] = [index, newIndex].sort((a, b) => a - b); // Sort the indixes
...
const isRequired = (
  ...
  const pathSegments: any = schemaPath.split('/');
      ...
      schema.allOf.forEach((allOfSchema) => {
        if (allOfSchema && allOfSchema.properties) {
          for (const key in allOfSchema.properties) {
            const property: any = allOfSchema.properties[key];
            const resolvedProperty = property.$ref
              ? Resolve.schema(rootSchema, property.$ref, rootSchema)
              : property;
            if (resolvedProperty && resolvedProperty.default !== undefined) {
...
export const mapStateToControlProps = (
  ...
  ownProps: OwnPropsOfControl | any
): StatePropsOfControl | any => {
  const { uischema }: any = ownProps;
...
): StatePropsOfMasterItem | any => {
  const { schema, path, uischema, childLabelProp, index }: any = ownProps;
...
export const mapStateToLayoutProps = (
  ...
  const label = isLabelable(uischema)
    ? deriveLabelForUISchemaElement(uischema as any, t)
    : undefined;
...
export const mapStateToCombinatorRendererProps = (
  ...
  const ajv = (state as any).jsonforms.core.ajv;
  ...
  let indexOfFittingSchema: number | any;
...
export const mapStateToLabelProps = (
  state: JsonFormsState,
  props: OwnPropsOfLabel | any
) => {
  const { uischema }: any = props;
...
export const computeChildLabel = (
  ...
  let enumOption: EnumOption | any = undefined;
