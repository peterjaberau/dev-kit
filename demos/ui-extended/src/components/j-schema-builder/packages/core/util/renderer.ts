...
export const mapStateToMasterListItemProps = (
  ...
  const { schema, path, index } : any = ownProps;
...
export const mapStateToArrayControlProps = (
  ...
  const { path, schema, uischema, i18nKeyPrefix, label, ...props }: any =
...
export const mapStateToLayoutProps = (
  ...
  const visible: boolean =
    ownProps.visible === undefined || hasShowRule(uischema)
      ? isVisible(ownProps.uischema as any, rootData, ownProps.path, getAjv(state))
      : ownProps.visible;
...
): StatePropsOfCombinator | any => {
  const { data, schema, rootSchema, ...props }: any = mapStateToControlProps(
    state,
    ownProps
  );

  const ajv: any = state.jsonforms.core.ajv;
