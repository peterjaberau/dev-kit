 ...
 */
export const resolveSchema: any = (
  schema: JsonSchema,
  schemaPath: string,
  rootSchema: JsonSchema
...
const resolvePathSegmentsWithCombinatorFallback = (
  ...
  const subSchemas = [].concat(
    ...
    schema.allOf ?? [],
    schema.anyOf ?? [],
    (schema as JsonSchema7 | any).then ?? [],
    (schema as JsonSchema7 | any).else ?? []
  );

  for (const subSchema of subSchemas as any) {
    let resolvedSubSchema: any = subSchema;
...
const resolveSingleStep = (
  ...
  const [segment, ...remainingPathSegments]: any = pathSegments;
