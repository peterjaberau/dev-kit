 ...
 */
export const convertDateToString = (date: Date, format?: "date" | "time" | "date-time"): string => {
  ...
  if (format === "date-time") {
    ...
    // e.g. '2023-11-09'
    return dateString.split("T")[0] as any
  } else if (format === "time") {
    //e.g. '14:22:54'
    // @ts-ignore
    return dateString.split("T")[1].split(".")[0] as any
  }
  return dateString
}
 ...
 */
export const deriveTypes = (jsonSchema: JsonSchema | any): string[] => {
  ...
  if (!isEmpty(jsonSchema.enum)) {
    ...
    jsonSchema.enum.forEach((enumElement: any) => {
...
// Paths --
const fromScoped = (scopable: Scoped): string => toDataPathSegments(scopable.scope).join(".")
