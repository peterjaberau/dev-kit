...
import { isRegularNode, RootNode, SchemaTree as JsonSchemaTree, SchemaTreeRefDereferenceFn } from "@stoplight/json-schema-tree"
...
import { JSVOptions, JSVOptionsContextProvider } from "../context"
...
const JsonSchemaViewerComponent = ({
  ...
  const options = React.useMemo(
    ...
    [defaultExpandedDepth, viewMode, onGoToRef, renderRowAddon, renderExtensionAddon, hideExamples, renderRootTreeLines, disableCrumbs, nodeHasChanged],
  )
...
const JsonSchemaViewerInner = ({
  ...
  JsonSchemaProps,
  "schema" | "viewMode" | "className" | "resolveRef" | "maxRefDepth" | "emptyText" | "onTreePopulated" | "maxHeight" | "parentCrumbs" | "skipTopLevelDescription"
>) => {
  ...
  return (
    ...
    >
      ...
      <TopLevelSchemaRow schemaNode={jsonSchemaTreeRoot.children[0] as any} skipDescription={skipTopLevelDescription} />
    </Box>
  )
}

const JsonSchemaFallbackComponent = ({ error }: any) => {
  return (
    <Box p={4}>
...
//@ts-ignore
export const JsonSchemaViewer = (props: JsonSchemaProps) => (
  <ErrorBoundary
    FallbackComponent={JsonSchemaFallbackComponent}
    resetKeys={[props.schema]} // equivalent to recoverableProps
  >
