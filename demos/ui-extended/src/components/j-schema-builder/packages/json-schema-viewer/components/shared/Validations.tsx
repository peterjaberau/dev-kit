...
const numberValidationFormatters: Record<string, (value: unknown) => string> | any = {
  minimum: (value: any) => `>= ${value}`,
  exclusiveMinimum: (value: any) => `> ${value}`,
  minItems: (value: any) => `>= ${value} items`,
  minLength: (value: any) => `>= ${value} characters`,
  maximum: (value: any) => `<= ${value}`,
  exclusiveMaximum: (value: any) => `< ${value}`,
  maxItems: (value: any) => `<= ${value} items`,
  maxLength: (value: any) => `<= ${value} characters`,
};
...
const validationFormatters: Record<string, (value: unknown) => ValidationFormat | null> | any = {
...
export const Validations: React.FunctionComponent<IValidations> = ({ validations, hideExamples }) => {
  ...
  return (
    <>
      <NumberValidations
        data-id='shared-number-validations'
        validations={numberValidations} />
      <KeyValueValidations
        data-id='shared-keyvalue-validations'
        validations={keyValueValidations} />
    </>
  );
};
...
const NumberValidations = ({
  ...
  return (
    <HStack color="muted" maxW="full" gap={1} data-test="property-validation">
      {entries
...
const KeyValueValidations = ({ validations }: { validations: Dictionary<unknown> }) => (
  <>
    {keys(validations)
      ...
      .map(key => {
        const validation: any = validationFormatters[key](validations[key]);
        if (validation) {
...
const getArrayValidations = (schemaNode: RegularNode | any) => {
