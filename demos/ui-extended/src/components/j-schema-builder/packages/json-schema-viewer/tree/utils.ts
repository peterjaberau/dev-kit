...
export function isFlattenableNode(node: SchemaNode): node is FlattenableNode {
  ...
  return (
    ...
    ((isRegularNode(node.children[0] as any) && (!isArrayNode(node) || !isDictionaryNode(node.children[0] as any))) ||
      (isReferenceNode(node.children[0] as any) && (node.children[0] as any).error !== null))
  );
}
...
export function isPropertyRequired(schemaNode: SchemaNode | any): boolean {
