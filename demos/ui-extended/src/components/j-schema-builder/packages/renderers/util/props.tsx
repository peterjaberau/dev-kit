...
import { convertToValidClassName, getAjv, getConfig } from "#jSchemaBuilder/core"
import { useJsonForms } from "#jSchemaBuilder/react"
...
export const addVanillaControlProps =
  ...
  (state: JsonFormsState, ownProps: OwnPropsOfControl): StatePropsOfControl & VanillaRendererProps => {
    ...
    let classNames: string[] = !isEmpty(controlElement.scope) ? styles.concat([`${convertToValidClassName(controlElement.scope)}`]) : [""]
...
export const withVanillaControlProps = (Component: ComponentType<any>) =>
  function WithVanillaControlProps(props: any) {
    ...
    return <Component {...props} getStyleAsClassName={getStyleAsClassName} getStyle={getStyle} classNames={classNamesProp} />
  }
...
const withVanillaCellPropsForType = (type: string) => (Component: ComponentType<any>) =>
  function WithVanillaCellPropsForType(props: any) {
    ...
    return <Component {...props} getStyleAsClassName={findStyleAsClassName(styles)} getStyle={findStyle(styles)} className={inputClassName.join(" ")} />
  }
