...
import { type CountableSlugify, slugifyWithCounter } from "@sindresorhus/slugify"
...
export const createOperationSlug = (slugify: CountableSlugify, operation: OperationLike) => {
  const summary = operation.summary || operation.operationId || `${operation.method}-${operation.path}`
...
const resolveExtensions = (obj: Record<string, any>) => Object.fromEntries(Object.entries(obj).filter(([key]) => key.startsWith("x-")))

export const getAllTags = (schema: OpenAPIDocument, slugs: ReturnType<typeof getAllSlugs>["tags"]): Array<Omit<TagObject, "name"> & { name?: string; slug?: string }> => {
  ...
  const hasUntaggedOperations = Object.values(schema.paths ?? {}).some((path) => Object.values(path ?? {}).some((op) => !(op as OperationObject).tags?.length))

  const result = [
    // Keep root tags that are actually used in operations
    ...rootTags.filter((tag) => operationTags.has(tag.name)).map((tag) => ({ ...tag, slug: slugs[tag.name] })),
...
export const getAllSlugs = (ops: GraphQLOperationObject[], schemaTags: TagObject[] = []) => {
  const slugify = slugifyWithCounter()

  const tags = Array.from(new Set([...ops.flatMap((op) => op.tags ?? []), ...schemaTags.map((tag) => tag.name)]))

  return {
    operations: Object.fromEntries(ops.map((op) => [getOperationSlugKey(op), createOperationSlug(slugify, op)])),
    tags: Object.fromEntries(tags.map((tag) => [tag, slugify(tag)])),
  }
}

const getOperationSlugKey = (op: GraphQLOperationObject) => [op.path, op.method, op.operationId, op.summary].filter(Boolean).join("-")

export const getAllOperations = (paths?: PathsObject): GraphQLOperationObject[] => {
...
const SchemaTag = builder.objectRef<Omit<TagObject, "name"> & { name?: string; slug?: string }>("SchemaTag")
...
const EncodingItem = builder.objectRef<EncodingObject & { name: string }>("EncodingItem").implement({
  fields: (t) => ({
    name: t.exposeString("name"),
...
const ExampleItem = builder.objectRef<ExampleObject & { name: string }>("ExampleItem").implement({
  fields: (t) => ({
    name: t.exposeString("name"),
...
const ParameterItem = builder.objectRef<ParameterObject>("ParameterItem").implement({
  fields: (t) => ({
    name: t.exposeString("name"),
    in: t.field({
      type: ParameterIn,
...
const OperationItem = builder.objectRef<GraphQLOperationObject>("OperationItem").implement({
  fields: (t) => ({
    slug: t.field({
      type: "String",
    ...
    servers: t.field({
      ...
      resolve: (parent, _, ctx) => {
        ...
        return parent.servers && parent.servers.length > 0 ? parent.servers : (ctx.schema.servers ?? [])
      },
    }),
    requestBody: t.field({
    ...
    tags: t.field({
      ...
      resolve: (parent, _, ctx) =>
        parent.tags?.map((tag) => ({
          name: tag,
          description: ctx.schema.tags?.find((t) => t.name === tag)?.description,
        })),
      nullable: true,
    }),
    deprecated: t.exposeBoolean("deprecated", { nullable: true }),
...
export const createGraphQLServer = (options?: Omit<YogaServerOptions<any, any>, "schema">) => createYoga({ schema, batching: true, ...options })
