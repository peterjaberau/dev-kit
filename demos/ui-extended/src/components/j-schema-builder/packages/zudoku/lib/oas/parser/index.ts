...
import { dereference, type JSONSchema } from "./dereference/index"
import { upgradeSchema } from "./upgrade/index"
...
type DeepOmitReference<T> = T extends OpenAPIV3_1.ReferenceObject ? never : T extends object ? { [K in keyof T]: DeepOmitReference<T[K]> } & WithRef : T
...
const parseSchemaInput = async (schemaInput: unknown): Promise<JSONSchema & { openapi?: string }> => {
