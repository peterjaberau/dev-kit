...
export function defaultDataProps(category: string, allFormInputs: any) {
  return allFormInputs[category].defaultDataSchema
}

export function defaultUiProps(category: string, allFormInputs: any) {
  return allFormInputs[category].defaultUiSchema
}

export function categoryType(category: string, allFormInputs: any) {
  return allFormInputs[category].type
}

export function getCardBody(category: string | any, allFormInputs: any) {
  return (allFormInputs[category] && allFormInputs[category].cardBody) || (() => null)
}

export function categoryToNameMap(allFormInputs: any) {
...
function updateElementNames(elementArray: any[]) {
  const elementNames = elementArray.map((elem: any) => elem.name)
...
export function generateCategoryHash(allFormInputs: any) {
  const categoryHash: any = {}
  Object.keys(allFormInputs).forEach((categoryName: any) => {
...
export function getCardCategory(cardProps: any, categoryHash: any) {
  const currentHash: any = `type:${cardProps.dataOptions.type || ""};widget:${cardProps.uiOptions["ui:widget"] || ""};field:${cardProps.uiOptions["ui:field"] || ""};format:${
    cardProps.dataOptions.format || ""
  };$ref:${cardProps.$ref !== undefined ? "true" : "false"};enum:${cardProps.dataOptions.enum ? "true" : "false"}`

  const category: any = categoryHash[currentHash]

  console.log('---getCardCategory---', { category })
...
function checkObjectForUnsupportedFeatures(
  schema: any,
  uischema: any,
  supportedWidgets: Set<string> | any,
  supportedFields: Set<string> | any,
  supportedOptions: Set<string> | any,
) {
      ...
      if (schema.dependencies) {
        Object.keys(schema.dependencies).forEach((dependencyKey) => {
          Object.keys(schema.dependencies[dependencyKey]).forEach((parameter) => {
            if (parameter === "oneOf") {
              schema.dependencies[dependencyKey].oneOf.forEach((grouping: { [key: string]: any }) => {
...
export function checkForUnsupportedFeatures(schema: any, uischema: any, allFormInputs: any) {
  // add each unsupported feature to this array
  ...
  Object.keys(allFormInputs).forEach((inputType) => {
    allFormInputs[inputType].matchIf.forEach((match: any) => {
...
export function generateElementPropsFromSchemas(parameters: any) {
  ...
  Object.entries(schema.properties).forEach(([parameter, element]) => {
    const newElement: any = {}
    let elementDetails: any = element && typeof element === "object" ? element : {}
  ...
  updateElementNames(cardPropList as any[])

  return cardPropList as any[]
}
...
function generateSchemaElementFromElement(element: any) {
...
export function generateSchemaFromElementProps(elementArr: any[]) {
  if (!elementArr) return {}
  const newSchema: any = {}

  const props: any = {}
  const dependencies: any = {}
  const elementDict: any = {}
  ...
  for (let index = 0; index < elementArr.length; index += 1) {
    ...
    if (element.dependents)
      element.dependents.forEach((possibility: any) => {
        possibility.children.forEach((dependentElement: any) => {
    ...
    if (element.dependents && element.dependents[0]) {
      if (element.dependents[0].value) {
        ...
        dependencies[elementName] = {
          oneOf: element.dependents.map((possibility: any) => {
            const childrenComponents: { [key: string]: any } = {}
            const requiredValues: string[] = []
            possibility?.children?.forEach((child: any) => {
        ...
        const childrenComponents: { [key: string]: any } = {}
        const requiredValues: string[] = []
        element.dependents[0].children.forEach((child: any) => {
          childrenComponents[child] = generateSchemaElementFromElement(elementDict[child])
          if (elementDict[child].required) requiredValues.push(child)
        })
        dependencies[elementName] = {
...
export function generateUiSchemaFromElementProps(elementArr: any, definitionUi: any): { [key: string]: any; definitions?: { [key: string]: any } } {
  if (!elementArr) return {}
...
export function getCardParameterInputComponentForType(category: string | any, allFormInputs: any) {
...
export function updateSchemas(elementArr: any[], parameters: any) {
  ...
  const newUiSchema: any = generateUiSchemaFromElementProps(elementArr, definitionUi)
...
function getIdFromElementsBlock(elements: any[]) {
...
export function addCardObj(parameters: any) {
...
export function addSectionObj(parameters: any) {
...
export function generateElementComponentsFromSchemas(parameters: any) {
  const {
    schemaData,
    uiSchemaData,
    onChange,
...
export function onDragEnd(result: any, details: any) {
...
function propagateElementChange(elementArr: any[], definitionData: any, definitionUi: any, categoryHash: any) {
  const updatedElementArr: any[] = []
...
export function propagateDefinitionChanges(schema: any, uischema: any, onChange: (schema: any, uischema: any) => void, categoryHash: any) {
...
export function excludeKeys(obj: any, keys?: string[] | null) {
  if (!keys) return { ...obj }

  // Create a map for performant object filtering
  const keysHash: any = keys.reduce(
    (acc, curr) => ({
      ...acc,
      [curr]: true,
    }),
...
export function getNewElementDefaultDataOptions(i: number, mods?: any) {
