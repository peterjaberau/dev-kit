...
export const generateCategoryHash = ({ context }) => {
  const { allFormInputs } = context
...
export const getCardCategory = ({ context, event }) => {
  const { categoryHash } = context
...
export const checkObjectForUnsupportedFeatures = ({ context, event }) => {
  const { schema, uiSchema, constants } = context
  const { supportedPropertyParameters, supportedUiParameters } = constants
  const { supportedWidgets, supportedFields, supportedOptions } = event
...
export const checkForUnsupportedFeatures = ({ context, event }) => {
  const { schema, uiSchema, allFormInputs, constants } = context
  const { supportedPropertyParameters, supportedUiParameters } = constants
  const { supportedWidgets, supportedFields, supportedOptions } = event
  ...
  if (schema && typeof schema === "object" && schema.type === "object") {
    unsupportedFeatures.push(...checkObjectForUnsupportedFeatures({ context, event: { supportedWidgets, supportedFields, supportedOptions }}))
  } else {
...
export const generateDependencyElement = ({ context, event }) => {
  const { schemaDefinitions, uiSchemaDefinitions } = context
  const { name, dataProps, uiProperties, requiredNames, useDefinitionDetails } = event

  let uiProps = { ...uiProperties }
  const newElement = {}
  ...
  if (elementDetails.$ref !== undefined && schemaDefinitions) {
    ...
    if (pathArr[0] === "#" && pathArr[1] === "definitions" && schemaDefinitions[pathArr[2]] && useDefinitionDetails === true) {
      elementDetails = {
        ...elementDetails,
  ...
  if (elementDetails.type && elementDetails.type === "object") {
    ...
    newElement.dataOptions!.category = getCardCategory({ context, event: newElement})
...
export function generateUiSchemaFromElementProps(elementArr: Array<ElementProps>, definitionUi: any): { [key: string]: any; definitions?: { [key: string]: any } } {
  if (!elementArr) return {}
...
export function getCardParameterInputComponentForType(category: string, allFormInputs: { [key: string]: FormInput }): CardComponentType {
...
function propagateElementChange(elementArr: ElementProps[], definitionData: { [key: string]: any }, definitionUi: { [key: string]: any }, categoryHash: { [key: string]: string }) {
