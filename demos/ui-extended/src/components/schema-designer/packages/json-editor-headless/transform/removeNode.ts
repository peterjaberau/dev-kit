...
import { split } from '#json-pointer';
...
export function removeNode<T extends Node = Node>(previousRoot: T, pointer: JsonPointer): [JsonError] | [T, Change[]] {
    ...
    const result: any = unlinkPath(previousRoot, frags);
    if (isJsonError(result)) {
        return [result];
    }

    const [nextRoot, parentNode]: any = result;
    ...
    const nodeToRemove: any = parentNode.children[removeNodeIndex];
    ...
    if (parentNode.type === 'array') {
        for (let i = removeNodeIndex, l = parentNode.children.length; i < l; i += 1) {
            parentNode.children[i] = updatePath(parentNode.children[i] as any, parentNode.pointer, `${i}`);
        }
    }
