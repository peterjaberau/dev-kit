...
export const formats: Record<string, (options: JsonSchemaValidatorParams) => undefined | JsonError | ValidationResult[]> = {
  ...
  email: ({ node, pointer, data }) => {
    ...
    if (/^\[.*\]$/.test(host)) {
      ...
      if (isValidIPV4.test(possibleIp) || isValidIPV6.test(possibleIp) || /IPv6:::[0-9a-f]{1,4}/.test(possibleIp)) {
        return undefined
      }
    }
  ...
  regex: ({ node, pointer, data }) => {
    const { schema } = node
    if (typeof data === "object" || typeof data === "number" || Array.isArray(data) || getTypeOf(data) === "boolean") {
      return undefined
    }
