...
> {
  ...
  onCopyIndexClick = (index: number) => {
    return (event: MouseEvent) => {
      ...
      const { onChange, errorSchema, fieldPathId } = this.props;
      const { keyedFormData }: any = this.state;
      // refs #195: revalidate to ensure properly reindexing errors
      let newErrorSchema: ErrorSchema<T>;
      if (errorSchema) {
        newErrorSchema = {};
  ...
  renderNormalArray() {
    ...
    const { OptionalDataControlsField }: any = fields;
    const uiOptions = getUiOptions<T[], S, F>(uiSchema);
    ...
    const actualFormData: any = hasFormData ? keyedFormData : [];
    ...
    const arrayProps: ArrayFieldTemplateProps<T[], S, F> = {
      canAdd,
      items: actualFormData.map((keyedItem: any, index: any) => {
