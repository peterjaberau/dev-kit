...
import { getTemplate, getUiOptions, ArrayFieldTemplateProps, ArrayFieldTemplateItemType, FormContextType, RJSFSchema, StrictRJSFSchema } from '@/components/module-rjsf/rjsf-utils';

/** The `ArrayFieldTemplate` component is the template used to render all items in an array.
 *
 * @param props - The `ArrayFieldTemplateItemType` props for the component
 */
export default function ArrayFieldTemplate<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any>(props: ArrayFieldTemplateProps) {
  ...
  return (
    <fieldset className={className} id={idSchema.$id}>
      ...
      <div className="row array-item-list">{items && items.map(({ key, ...itemProps }: ArrayFieldTemplateItemType) => <ArrayFieldItemTemplate key={key} {...itemProps} />)}</div>
