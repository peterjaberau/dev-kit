 ...
 */
export default function NativeSelectWidget<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any>(props: WidgetProps<T, S, F>) {
  const {
    id,
    options,
    label,
  ...
  const _onBlur = ({ target }: FocusEvent<HTMLSelectElement>) => onBlur(id, enumOptionsValueForIndex<S>(target && target.value, enumOptions, emptyValue))

  const _onFocus = ({ target }: FocusEvent<HTMLSelectElement>) => onFocus(id, enumOptionsValueForIndex<S>(target && target.value, enumOptions, emptyValue))
  ...
  return (
    ...
    >
      <ChakraSelect.Root>
        <ChakraSelect.Field id={id} onBlur={_onBlur} onChange={_onChange} onFocus={_onFocus} autoFocus={autofocus} value={formValue} aria-describedby={ariaDescribedByIds(id)}>
